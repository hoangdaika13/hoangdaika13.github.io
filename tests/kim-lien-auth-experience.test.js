...
test("Kim Liên login builds a solemn CSS sanctuary without touching auth data", () => {
  ...
  assert.match(client, /kl-sanctum-devotion/);
  assert.match(client, /Tâm an tịnh, trí sáng trong; mỗi bước chân là một đóa sen nở/);
...
test("Kim Liên login remains opaque, scrollable and readable at narrow widths", () => {
  ...
  assert.match(css, /@layer pages[\s\S]{0,420}?\.kl-centered-devotional-host[\s\S]{0,120}?display:\s*grid\s*!important/);
