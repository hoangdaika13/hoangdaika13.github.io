...
test("Cosmic Studio is registered as a lazy Galaxy module", () => {
  ...
  assert.match(loader, /galaxy-living-universe\.js\?v=5/);
...
test("new assets are available to the offline catalog", () => {
  ...
  assert.match(worker, /galaxy-living-universe\.js\?v=5/);
  assert.match(worker, /galaxy-universe-renderer\.mjs\?v=5/);
  assert.match(worker, /galaxy-home-ai\.css\?v=25/);
  assert.match(worker, /performance-loader\.js\?v=682/);
});
