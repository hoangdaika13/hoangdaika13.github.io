...
test("Cosmic Studio is registered as a lazy Galaxy module", () => {
  ...
  assert.match(loader, /galaxy-living-universe\.js\?v=7/);
  assert.match(loader, /galaxy-living-universe\.css\?v=3/);
...
test("new assets are available to the offline catalog", () => {
  ...
  assert.match(worker, /galaxy-living-universe\.js\?v=7/);
  assert.match(worker, /galaxy-living-universe\.css\?v=3/);
  assert.match(worker, /galaxy-universe-renderer\.mjs\?v=7/);
  assert.match(worker, /galaxy-home-ai\.css\?v=25/);
  assert.match(worker, /performance-loader\.js\?v=685/);
});
