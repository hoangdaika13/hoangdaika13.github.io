...
test("login mounts a real WebGL living galaxy without replacing authentication", () => {
  ...
  assert.match(html, /auth-living-galaxy-3d\.css\?v=16/);
  assert.match(gateway, /auth-living-galaxy-3d\.js\?v=22/);
  ...
  assert.match(runtime, /makeStarTexture/);
  assert.match(runtime, /makeNebulaTexture/);
  assert.match(runtime, /makeMilkyWayTexture/);
  assert.match(runtime, /const temperatures = \[0xfff4de, 0xddeeff, 0xa9cfff, 0xffd4ad, 0xffffff\]/);
  ...
  assert.match(runtime, /orbitAngle = \(planet\.orbitAngle \+ delta \* planet\.speed \* warpBoost \* planet\.orbitRate\) % ORBIT_TAU/);
