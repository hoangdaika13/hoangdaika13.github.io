...
test("Creative Galaxy assets are versioned and precached", () => {
  ...
  for (const asset of [
    "creative-galaxy.css?v=1", "creative-galaxy.js?v=1", "creative-os-core.js?v=2",
    "creative-os.js?v=4", "ai-center-advanced.js?v=2"
  ]) {
    const pattern = new RegExp(asset.replace(/[.?]/g, "\\$&"));
    assert.match(`${loader}\n${worker}`, pattern);
  }
  assert.match(worker, /hh-identity-portal-v244/);
  assert.match(html, /performance-loader\.js\?v=37/);
  assert.match(html, /script\.js\?v=132/);
});
