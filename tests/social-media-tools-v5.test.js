...
test("Tool Contract V7 covers every catalog item and never marks provider-only tools ready",()=>{const all=contracts.catalogContracts(core.TOOL_CATALOG);assert.equal(contracts.CONTRACT_VERSION,7);assert.equal(all.length,85);assert.ok(all.every(item=>Array.isArray(item.inputs)&&item.exports.length&&item.validator==="tool-specific"));assert.equal(contracts.contractFor(core.TOOL_CATALOG.find(t=>t.id==="bio-link")).readiness.code,"local-ready");assert.equal(contracts.contractFor(core.TOOL_CATALOG.find(t=>t.id==="alt-text-checker")).readiness.code,"local-ready");assert.equal(contracts.contractFor(core.TOOL_CATALOG.find(t=>t.id==="analytics")).readiness.operational,false);});
