...
test("Admin Panel is a top-level admin-only area guarded by server-projected access", () => {
  ...
  assert.match(adminClient, /PLANETS[\s\S]*Mission Control[\s\S]*Identity & Access[\s\S]*Security Center[\s\S]*Users & Community[\s\S]*Content & Trust[\s\S]*Platform & Release[\s\S]*Growth & Data/);
  assert.match(adminClient, /hh\.admin-galaxy\.preferences\.v1/);
  assert.match(adminClient, /data-admin-motion-select/);
  assert.match(adminClient, /prefers-reduced-motion/);
  assert.match(adminApi, /user:roles/);
  assert.match(adminApi, /user:revoke-sessions/);
  assert.match(adminApi, /content:moderate/);
  assert.match(adminApi, /incident:update/);
  assert.match(adminApi, /queue-job:update/);
  ...
  assert.match(adminClient, /Users & Community/);
