...
test("Discord is a top-level lazy workspace across shell, home and auth galaxy", () => {
  ...
  assert.match(loader, /discord:\s*\{[\s\S]*?discord-hub\.css\?v=4[\s\S]*?discord-hub\.js\?v=4/);
...
test("Discord Center has real OAuth, server, channel, message and safe composer flows", () => {
  ...
  for (const action of ["oauth/start", "guilds", "channels/", "messages/send", "messages/upload", "reactions", "threads", "audit", "bot/invite", "disconnect"]) assert.ok(hub.includes(action), `missing ${action}`);
  assert.match(hub, /credentials: "include"/);
  assert.match(hub, /SYNC_ACTIVE_MS = 5000/);
  assert.match(hub, /SYNC_IDLE_MS = 12000/);
  assert.match(hub, /visibilitychange/);
  assert.match(hub, /data-dh-composer/);
  assert.match(hub, /data-dh-save-message/);
  assert.match(hub, /data-dh-thread-create/);
  assert.match(hub, /data-dh-pref="notifications"/);
...
test("Discord backend keeps tokens private and sends bot messages without automatic mentions", () => {
  ...
  assert.match(manager, /DISCORD_MESSAGE_NOT_OWNED/);
  assert.match(manager, /ATTACHMENT_LIMIT = 1_500_000/);
  assert.match(manager, /discordAuditEvents[\s\S]*expiresAt/);
