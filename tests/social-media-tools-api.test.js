...
test("route, lazy loader, service worker and UI behavior are integrated", () => {
  ...
  const uiV2=read("social-media-tools-v2.js"), cssV2=read("social-media-tools-v2.css"), previewCss=read("social-media-preview-runtime.css"), previewRuntime=read("social-media-preview-runtime.js");
  assert.match(script,/route: "\/social-media-tools"/); assert.match(script,/HHSocialMediaTools\.mount/); assert.match(loader,/social-media-tools-core\.js\?v=8/); assert.match(loader,/social-media-communication-engines\.js\?v=1/); assert.match(loader,/social-media-tool-contracts\.js\?v=7/); assert.match(loader,/social-media-local-engines\.js\?v=6/); assert.match(loader,/social-media-pipeline\.js\?v=6/); assert.match(loader,/social-media-tool-capabilities\.js\?v=7/); assert.match(loader,/social-media-preview-runtime\.js\?v=1/); assert.match(sw,/social-media-tools-v2\.js\?v=13/);
  ...
  assert.match(uiV2,/stageProjectUpdate/);assert.match(uiV2,/data-smt2-preview-safe/);assert.match(uiV2,/data-smt2-preview-fullscreen/);assert.match(previewRuntime,/requestAnimationFrame|defaultRaf/);assert.match(previewRuntime,/visibilitychange/);assert.match(previewCss,/\.smt2-preview-toolbar/);assert.match(previewCss,/data-preview-bg/);
...
test("analytics, inbox and AI copilots are owner scoped and never fake publish",()=>{const backend=read("utils/social-media-handler.js");assert.match(backend,/social_analytics_snapshots/);assert.match(backend,/native-metrics-are-not-summed-across-incompatible-providers/);assert.match(backend,/social_inbox_items/);assert.match(backend,/autoReply:false/);assert.match(backend,/pending-human-review/);assert.match(backend,/autoPublish:false/);assert.match(backend,/requestId:randomBytes/);});
