...
test("router, lazy loader and responsive workspace are integrated", () => {
  ...
  const css = read("focus-study-room.css");
  const source = read("focus-study-room.js");
  assert.match(router, /id: "focus-room"[\s\S]*?route: "\/focus-room"/);
  assert.match(router, /groupIds: \["learn", "focus-room", "english"/);
  assert.match(router, /HHFocusStudyRoom\?\.mount/);
  assert.match(loader, /"focus-study-room": \{[\s\S]*?focus-study-room\.css\?v=2[\s\S]*?focus-study-room\.js\?v=1/);
