...
test("router, lazy loader and responsive workspace are integrated", () => {
  ...
  assert.match(loader, /"focus-study-room": \{[\s\S]*?focus-room\.css\?v=10[\s\S]*?focus-room\.js\?v=10/);
  ...
  assert.match(index, /performance-loader\.js\?v=661/);
  assert.match(index, /script\.js\?v=276/);
  assert.match(worker, /performance-loader\.js\?v=661/);
  assert.match(worker, /script\.js\?v=276/);
