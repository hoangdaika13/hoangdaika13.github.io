...
module.exports = async function handler(req, res) {
  return withApi(req, res, async ({ db, body }) => {
    ...
    if (req.method === "GET") {
      ...
      return res.status(200).json({
        ...
        capabilities: {
          ...
          largeBinaryFiles: false,
          objectStorage: false,
          message: "File lớn cần kết nối S3, Cloudflare R2 hoặc GridFS."
        }
      });
    }
    ...
    return res.status(200).json({
      ok: true,
      file: { ...doc, _id: result.insertedId },
      capabilities: { metadata: true, smallTextPayload: true, largeBinaryFiles: false, objectStorage: false }
    });
  });
};
