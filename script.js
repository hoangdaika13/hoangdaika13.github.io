...
function initSuperPlatform() {
  ...
  const communityDialog=({title="HH Social",message="",fields=[],actions=[{id:"cancel",label:"Hủy"},{id:"confirm",label:"Xác nhận",primary:true}]})=>new Promise((resolve)=>{document.querySelector("[data-hh-community-dialog]")?.remove();const dialog=document.createElement("dialog");dialog.className="hh-community-dialog";dialog.dataset.hhCommunityDialog="";dialog.innerHTML=`<section><header><div><span>HH SOCIAL</span><h5>${escapeHtml(title)}</h5></div><button type="button" data-dialog-close aria-label="Đóng">×</button></header>${message?`<p>${escapeHtml(message)}</p>`:""}<div class="hh-dialog-fields">${fields.map((field)=>`<label><span>${escapeHtml(field.label||"")}</span>${field.type==="textarea"?`<textarea name="${escapeHtml(field.name)}" rows="4" placeholder="${escapeHtml(field.placeholder||"")}">${escapeHtml(field.value||"")}</textarea>`:`<input name="${escapeHtml(field.name)}" type="${escapeHtml(field.type||"text")}" value="${escapeHtml(field.value||"")}" placeholder="${escapeHtml(field.placeholder||"")}">`}</label>`).join("")}</div><footer>${actions.map((action)=>`<button type="button" class="${action.primary?"primary":""} ${action.danger?"danger":""}" data-dialog-action="${escapeHtml(action.id)}">${escapeHtml(action.label)}</button>`).join("")}</footer></section>`;document.body.append(dialog);const finish=(action)=>{const values=Object.fromEntries([...dialog.querySelectorAll("[name]")].map((field)=>[field.name,field.value.trim()]));dialog.close();dialog.remove();resolve({action,values});};dialog.addEventListener("cancel",(event)=>{event.preventDefault();finish("cancel");},{once:true});dialog.addEventListener("click",(event)=>{if(event.target===dialog||event.target.closest("[data-dialog-close]"))finish("cancel");const button=event.target.closest("[data-dialog-action]");if(button)finish(button.dataset.dialogAction);});dialog.showModal();setTimeout(()=>dialog.querySelector("input,textarea,button")?.focus(),40);});
  ...
  const communityStoryMarkup=(story)=>`<button class="community-story interactive" type="button" data-story-id="${escapeHtml(story.id)}" data-story-content="${escapeHtml(story.content||"")}" data-story-media="${escapeHtml(story.mediaUrl||"")}">${story.mediaUrl?`<img src="${escapeHtml(story.mediaUrl)}" alt="" loading="lazy">`:""}<span>${communityInitials(story.author?.name)}</span><strong>${escapeHtml(story.author?.name||"Thành viên HH")}</strong><small>${communityTime(story.createdAt)}</small></button>`;
  ...
  grid.addEventListener("click", (event) => {
    ...
    const community=event.target.closest("[data-community-center]");if(community){
      ...
      if(event.target.closest("[data-story-create]")){(async()=>{const result=await communityDialog({title:"Tạo Tin mới",message:"Tin được lưu an toàn và tự biến mất sau 24 giờ.",fields:[{name:"content",label:"Nội dung",type:"textarea",placeholder:"Chia sẻ một khoảnh khắc..."},{name:"mediaUrl",label:"Ảnh (liên kết HTTPS)",type:"url",placeholder:"https://..."}],actions:[{id:"cancel",label:"Hủy"},{id:"create",label:"Đăng Tin",primary:true}]});if(result.action!=="create")return;if(!result.values.content&&!result.values.mediaUrl)return communityNotice("Hãy nhập nội dung hoặc thêm ảnh.","error");await communityMutate({action:"story:create",content:result.values.content,mediaUrl:result.values.mediaUrl});communityNotice("Tin đã được đăng và sẽ hết hạn sau 24 giờ.","success");})().catch((error)=>communityNotice(error.message,"error"));return;}
    }
