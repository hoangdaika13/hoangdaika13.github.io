...
const API_URL = String(window.HH_API_BASE || REALTIME_URL || location.origin).replace(/\/$/, "");
...
const revokeCurrentSession = async () => {
  ...
  try {
    await fetch(`${REALTIME_URL}/api/auth/logout`, { method: "POST", credentials: "include", headers: token ? { Authorization: `Bearer ${token}` } : {}, cache: "no-store" });
...
function initPublicAuthPanel() {
  ...
  const authenticate = async (path, payload) => {
    ...
    window.HHAuthSession?.setToken?.(data.token);
...
function initRealtimeAuth() {
  if (window.HHAuthPlatform?.init) {
    return window.HHAuthPlatform.init({
  ...
  let token = window.HHAuthSession?.token?.() || "";
  let user = null;
  ...
  const updateOAuthButtons = () => {
    oauthButtons.forEach((button) => {
      ...
      const enabled = Boolean(provider === "google" && REALTIME_URL && oauthProviders?.google);
      button.disabled = !enabled;
      button.title = enabled ? "Đăng nhập bằng Google" : "Google chưa được cấu hình trên Vercel";
    });
  };
  ...
  const handleRegister = async (event, formNode) => {
    ...
    try {
      ...
      token = data.token;
      user = data.user;
      window.HHAuthSession?.setToken?.(token);
...
function initSuperPlatform() {
  ...
  const readAIState = () => {
    ...
    const sessions = (Array.isArray(state.sessions) ? state.sessions : []).map((session, index) => ({
      id: session.id || `ai-legacy-${index}`,
      title: session.title || "Phiên Gemini",
      createdAt: session.createdAt || session.time || new Date().toISOString(),
      updatedAt: session.updatedAt || session.time || new Date().toISOString(),
      messages: Array.isArray(session.messages) ? session.messages : [
        ...(session.input ? [{ id: `u-${index}`, role: "user", text: session.input, time: session.time || "" }] : []),
        ...(session.result ? [{ id: `m-${index}`, role: "assistant", text: session.result, time: session.time || "", provider: "legacy" }] : [])
      ]
    }));
    if (!sessions.length) sessions.push({ id: "ai-new", title: "Cuộc trò chuyện mới", createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(), messages: [] });
    return {
      ...state,
  ...
  const writeAIState = (state) => localStorage.setItem("hh-ai-center", JSON.stringify(state));
  const aiActiveSession = (state = readAIState()) => state.sessions.find((session) => session.id === state.activeSessionId) || state.sessions[0];
  const aiSafeSourceUrl = (value) => {
  ...
  const aiCenterMarkup = () => {
    ...
    const activeSession = aiActiveSession(state);
    return `
      <section class="ai-center-app ai-center-pro" data-ai-center>
        ...
        <div class="ai-center-layout">
          <aside class="ai-sidebar">
            ...
            <div class="ai-session-list" data-ai-sessions>${aiSessionListMarkup(state)}</div>
          ...
          <aside class="ai-context-panel">
            ...
            <details class="ai-runtime-settings" open><summary>Cấu hình hội thoại</summary><label>Chỉ dẫn hệ thống<textarea rows="4" data-ai-system-prompt>${escapeHtml(state.systemPrompt)}</textarea></label><label>Sáng tạo <span data-ai-creativity-value>${state.creativity}%</span><input type="range" min="20" max="120" value="${state.creativity}" data-ai-creativity></label><label class="ai-toggle"><input type="checkbox" data-ai-google-search ${state.useGoogleSearch ? "checked" : ""}><span>Tìm kiếm Google cho câu trả lời mới</span></label><div class="ai-runtime-meta"><span data-ai-provider>Provider: đang kiểm tra</span><span data-ai-usage>Token: --</span><span data-ai-latency>Độ trễ: --</span></div></details>
          </aside>
        </div>
      </section>`;
  };
  ...
  const mediaCenterMarkup = () => {
    ...
    const defaults = [
      ...
      { id: "hh-profile", title: "Nhhoang · HH Neon Platform", type: "link", category: "gallery", url: "https://hoang8.com", source: "Website", favorite: false }
    ];
...
const securityCenterMarkup=()=>`<section class="security-app" data-security><header class="suite-hero"><div><p class="section-kicker">Security Center 19</p><h4>Bảo mật tài khoản</h4><span>Lịch sử đăng nhập, Passkey, phiên thiết bị và quyền dữ liệu cá nhân.</span></div><div class="security-score"><strong data-security-score>72</strong><span>Security score</span></div></header><div class="security-alert" data-security-alert><i></i><div><strong>Đang kiểm tra bảo mật tài khoản</strong><span>Phiên, thiết bị và Passkey được xác minh trực tiếp với máy chủ.</span></div></div><div class="security-layout"><aside class="security-nav">${[["overview","Tổng quan"],["history","Lịch sử đăng nhập"],["sessions","Phiên & thiết bị"],["passkeys","Passkey"],["permissions","Quyền dữ liệu"],["audit","Audit logs"]].map(([id,label],index)=>`<button class="interactive ${index===0?"active":""}" type="button" data-security-tab="${id}">${label}</button>`).join("")}</aside><main class="security-workspace"><section class="security-pane active" data-security-pane="overview"><div class="security-checks">${[["Mật khẩu tài khoản","Đã thiết lập","safe"],["Email đăng nhập","Đang kiểm tra","safe"],["Passkey","Có thể thêm sau khi đăng nhập","warn"],["Phiên hiện tại","Đang hoạt động","safe"]].map((item)=>`<article class="${item[2]}"><i></i><div><strong>${item[0]}</strong><span>${item[1]}</span></div></article>`).join("")}</div><button class="button ghost interactive" type="button" data-security-check>Kiểm tra lại tài khoản</button></section><section class="security-pane" data-security-pane="history"><div class="login-history" data-login-history><p>Đang tải lịch sử đăng nhập...</p></div></section><section class="security-pane" data-security-pane="sessions"><div class="security-session-list" data-security-sessions><p>Đang tải các phiên đã đăng nhập...</p></div><button class="button ghost interactive" type="button" data-security-logout-all>Đăng xuất khỏi thiết bị khác</button></section><section class="security-pane" data-security-pane="passkeys"><div class="security-passkey-head"><div><strong>Passkey của bạn</strong><span>Dùng vân tay, khuôn mặt hoặc PIN; không gửi sinh trắc học lên HH.</span></div><button class="button primary interactive" type="button" data-security-passkey-add>+ Tạo Passkey</button></div><div class="security-passkey-list" data-security-passkeys><p>Đang tải Passkey...</p></div></section><section class="security-pane" data-security-pane="permissions"><div class="permission-list">${["Lưu hồ sơ cá nhân","Lưu lịch sử module","Thông báo trong ứng dụng","Analytics cục bộ"].map((item,index)=>`<label><span>${item}</span><input type="checkbox" data-security-permission="${index}" checked><i></i></label>`).join("")}</div></section><section class="security-pane" data-security-pane="audit"><pre data-security-audit>Audit logs chỉ hiển thị hành động thuộc tài khoản hiện tại.</pre></section></main></div></section>`;
  ...
  const renderAIConversation = () => {
    ...
    const list = panel.querySelector("[data-ai-sessions]");
    const stream = panel.querySelector("[data-ai-stream]");
    if (list) list.innerHTML = aiSessionListMarkup(state);
    if (stream) {
  ...
  const renderAIAttachments = () => {
    const tray = aiPanel()?.querySelector("[data-ai-attachments]");
    if (!tray) return;
    tray.hidden = !aiPendingAttachments.length;
    tray.innerHTML = aiPendingAttachments.map((file, index) => `<span><b>${file.mimeType.startsWith("image/") ? "Ảnh" : "TXT"}</b>${escapeHtml(file.name)}<button type="button" data-ai-remove-attachment="${index}" aria-label="Bỏ tệp">×</button></span>`).join("");
  };
  const loadAICenterStatus = async () => {
    const panel = aiPanel();
    if (!panel) return;
    const base = String(window.HH_REALTIME_URL || location.origin).replace(/\/$/, "");
    const token = window.HHAuthSession?.token?.() || "";
    const anonymousId = creativeAnonymousId();
    try {
      const response = await fetch(`${base}/api/modules/ai-center/actions?anonymousId=${encodeURIComponent(anonymousId)}`, {
        headers: token ? { Authorization: `Bearer ${token}` } : {},
        cache: "no-store"
      });
      const data = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(data.error || "Không kết nối được backend");
      panel.querySelector("[data-ai-provider-state]")?.setAttribute("data-ai-provider-state", data.configured ? "online" : "local");
      ...
      const status = panel.querySelector("[data-ai-status]");
      const detail = panel.querySelector("[data-ai-status-detail]");
      if (status) status.textContent = "Backend chưa phản hồi";
  ...
  const aiSaveSession = (title, input, result) => {
    ...
    if (input) session.messages.push({ id: `u-${Date.now()}`, role: "user", text: input, time: new Date().toLocaleTimeString("vi-VN") });
    if (result && !session.messages.some((message) => message.text === result)) session.messages.push({ id: `m-${Date.now()}`, role: "assistant", text: result, time: new Date().toLocaleTimeString("vi-VN"), provider: "saved" });
    session.updatedAt = new Date().toISOString();
    writeAIState(state);
    renderAIConversation();
    return session;
  };
  const aiRunChat = async (form, overrideText = "") => {
    const panel = aiPanel();
    ...
    session.title = session.messages.length ? session.title : text.slice(0, 52);
    session.messages.push({ id: `u-${Date.now()}`, role: "user", text: `${text}${aiPendingAttachments.length ? `\n[Đính kèm: ${aiPendingAttachments.map((file) => file.name).join(", ")}]` : ""}`, time: new Date().toLocaleTimeString("vi-VN") });
    session.updatedAt = new Date().toISOString();
    state.model = model;
    state.systemPrompt = panel?.querySelector("[data-ai-system-prompt]")?.value.trim() || state.systemPrompt;
    state.creativity = Number(panel?.querySelector("[data-ai-creativity]")?.value || 72);
  ...
  grid.addEventListener("keydown", (event) => {
    if (event.target.matches("[data-ai-chat-input]") && event.key === "Enter" && !event.shiftKey) {
      event.preventDefault();
      event.target.closest("[data-ai-chat-form]")?.requestSubmit();
    }
    if (event.target.matches("[data-ai-delete-session]") && (event.key === "Enter" || event.key === " ")) {
      event.preventDefault();
      event.target.click();
    }
  });
  ...
  grid.addEventListener("input", (event) => {
    ...
    if (event.target.matches("[data-ai-creativity]")) {
      const label = aiPanel()?.querySelector("[data-ai-creativity-value]");
...
const communityForm=event.target.closest("[data-community-form]");if(communityForm){event.preventDefault();const input=communityForm.querySelector("[data-community-input]");const content=input?.value.trim();const mediaUrl=communityForm.querySelector("[data-community-media]")?.value.trim();const pending=communityComposerFiles.get(communityForm);const files=pending?.files||[];if(!content&&!mediaUrl&&!files.length)return input?.focus();const option=(name,fallback="")=>communityForm.querySelector(`[data-v2-composer-value="${name}"]`)?.value??fallback;const jsonOption=(name)=>{try{const value=JSON.parse(option(name,"[]"));return Array.isArray(value)?value:[];}catch{return [];}};const button=communityForm.querySelector('[type="submit"]');button.disabled=true;button.textContent=files.length?`Đang tải 0/${files.length}...`:"Đang đăng...";(async()=>{try{const mediaIds=[];for(let index=0;index<files.length;index+=1){button.textContent=`Đang tải ${index+1}/${files.length}...`;const uploaded=await communityUploadMedia(files[index]);mediaIds.push(uploaded.media.id);}button.textContent="Đang đăng...";await communityMutate({action:"create",content,mediaUrl,mediaIds,mediaType:communityForm.querySelector("[data-community-media-type]")?.value,topic:communityForm.querySelector("[data-community-category]")?.value,privacy:communityForm.querySelector("[data-community-privacy]")?.value,audienceIncludeIds:jsonOption("audienceIncludeIds"),audienceExcludeIds:jsonOption("audienceExcludeIds"),feeling:communityForm.querySelector("[data-community-feeling-value]")?.value||"",location:communityForm.querySelector("[data-community-location-value]")?.value||"",pollQuestion:communityForm.querySelector("[data-community-poll-question]")?.value||"",pollOptions:[...communityForm.querySelectorAll("[data-community-poll-option]")].map((field)=>field.value.trim()).filter(Boolean),scheduledAt:option("scheduledAt"),background:option("background"),canReshare:option("canReshare","true")!=="false",commentsEnabled:option("commentsEnabled","true")!=="false",hideReactionCounts:option("hideReactionCounts","false")==="true"});input.value="";const media=communityForm.querySelector("[data-community-media]");if(media)media.value="";const picker=communityForm.querySelector("[data-community-file]");if(picker)picker.value="";communityRenderComposerFiles(communityForm,[]);communityForm.querySelector("[data-community-media-fields]").hidden=true;communityNotice("Bài viết đã xuất hiện trên bảng tin.","success");localStorage.removeItem("hh-community-draft");communityForm.querySelector("[data-social-composer-extra]")?.remove();communityForm.querySelectorAll("[data-community-feeling-value],[data-community-location-value],[data-v2-composer-value]").forEach((field)=>field.remove());}catch(error){communityNotice(error.message,"error");}finally{button.disabled=false;button.textContent="Đăng";}})();return;}
  ...
  grid.addEventListener("change", (event) => {
    const aiAttachment = event.target.closest("[data-ai-attach]");
    if (aiAttachment) {
      const files = Array.from(aiAttachment.files || []).slice(0, 4);
      (async () => {
        for (const file of files) {
          if (file.type.startsWith("image/")) {
            if (aiPendingAttachments.filter((item) => item.mimeType.startsWith("image/")).length >= 2) continue;
            if (!/^image\/(png|jpeg|webp|gif)$/.test(file.type) || file.size > 1_500_000) continue;
            ...
            aiPendingAttachments.push({ name: file.name, mimeType: file.type, size: file.size, data: dataUrl.split(",")[1] || "" });
          } else if (file.size <= 256_000) {
            const input = aiPanel()?.querySelector("[data-ai-chat-input]");
            const content = await file.text();
            if (input) input.value = `${input.value}${input.value ? "\n\n" : ""}--- ${file.name} ---\n${content.slice(0, 12000)}`;
            aiPendingAttachments.push({ name: file.name, mimeType: file.type || "text/plain", size: file.size, data: "" });
          }
        }
        renderAIAttachments();
        aiAttachment.value = "";
      })().catch((error) => aiResult("Không thể đọc tệp", error.message));
      return;
    }
    const aiSetting = event.target.closest("[data-ai-model],[data-ai-system-prompt],[data-ai-creativity],[data-ai-google-search]");
    if (aiSetting) {
      const panel = aiSetting.closest("[data-ai-center]");
      const state = readAIState();
      state.model = panel?.querySelector("[data-ai-model]")?.value || state.model;
      state.systemPrompt = panel?.querySelector("[data-ai-system-prompt]")?.value.trim() || state.systemPrompt;
      state.creativity = Number(panel?.querySelector("[data-ai-creativity]")?.value || state.creativity);
      state.useGoogleSearch = Boolean(panel?.querySelector("[data-ai-google-search]")?.checked);
      writeAIState(state);
      return;
    }
  ...
  grid.addEventListener("click", async (event) => {
    ...
    const security=event.target.closest("[data-security]");if(security){const tab=event.target.closest("[data-security-tab]");if(tab){security.querySelectorAll("[data-security-tab]").forEach((item)=>item.classList.toggle("active",item===tab));security.querySelectorAll("[data-security-pane]").forEach((item)=>item.classList.toggle("active",item.dataset.securityPane===tab.dataset.securityTab));if(tab.dataset.securityTab==="sessions")loadSecuritySessions(security);if(tab.dataset.securityTab==="passkeys")loadSecurityPasskeys(security);return;}const revokeSession=event.target.closest("[data-security-session-revoke]");if(revokeSession){revokeSession.disabled=true;securityAuthApi("/api/auth/session-revoke",{method:"POST",body:JSON.stringify({sessionId:revokeSession.dataset.securitySessionRevoke})}).then(()=>loadSecuritySessions(security)).catch((error)=>{revokeSession.disabled=false;security.querySelector("[data-security-alert] span").textContent=error.message;});return;}const revokePasskey=event.target.closest("[data-security-passkey-revoke]");if(revokePasskey){revokePasskey.disabled=true;securityAuthApi("/api/auth/passkey-revoke",{method:"POST",body:JSON.stringify({credentialId:revokePasskey.dataset.securityPasskeyRevoke})}).then(()=>loadSecurityPasskeys(security)).catch((error)=>{revokePasskey.disabled=false;security.querySelector("[data-security-alert] span").textContent=error.message;});return;}if(event.target.closest("[data-security-passkey-add]")){const add=event.target.closest("[data-security-passkey-add]");add.disabled=true;add.textContent="Đang tạo...";addSecurityPasskey(security).then(()=>{add.textContent="+ Tạo Passkey";security.querySelector("[data-security-alert] span").textContent="Passkey đã được thêm vào tài khoản.";}).catch((error)=>{add.textContent="+ Tạo Passkey";security.querySelector("[data-security-alert] span").textContent=error.name==="NotAllowedError"?"Bạn đã hủy tạo Passkey.":error.message;}).finally(()=>{add.disabled=false;});return;}if(event.target.closest("[data-security-logout-all]")){(async()=>{try{const data=await securityAuthApi("/api/auth/sessions");await Promise.all((data.sessions||[]).filter((item)=>!item.current).map((item)=>securityAuthApi("/api/auth/session-revoke",{method:"POST",body:JSON.stringify({sessionId:item.id})})));await loadSecuritySessions(security);security.querySelector("[data-security-alert] span").textContent="Đã đăng xuất khỏi mọi thiết bị khác.";}catch(error){security.querySelector("[data-security-alert] span").textContent=error.message;}})();return;}if(event.target.closest("[data-security-check]")){loadSecurityData();security.querySelector("[data-security-alert] strong").textContent="Đang xác minh phiên với máy chủ...";return;}if(event.target.closest("[data-security-logout]")){(async()=>{await revokeCurrentSession();window.HHAuthSession?.setToken?.("");localStorage.removeItem("hh-auth-user");localStorage.removeItem("hh-community-center");Object.keys(localStorage).filter((key)=>key.startsWith("hh-community-center:")).forEach((key)=>localStorage.removeItem(key));location.reload();})();return;}}
    ...
    if (ai) {
      ...
      const deleteSession = event.target.closest("[data-ai-delete-session]");
      if (deleteSession) {
        event.stopPropagation();
        const state = readAIState();
        state.sessions = state.sessions.filter((session) => session.id !== deleteSession.dataset.aiDeleteSession);
        if (!state.sessions.length) state.sessions.push({ id: `ai-${Date.now()}`, title: "Cuộc trò chuyện mới", createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(), messages: [] });
      ...
      if (event.target.closest("[data-ai-new]")) {
        const state = readAIState();
        const session = { id: `ai-${Date.now()}`, title: "Cuộc trò chuyện mới", createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(), messages: [] };
      ...
      const removeAttachment = event.target.closest("[data-ai-remove-attachment]");
      if (removeAttachment) {
        aiPendingAttachments.splice(Number(removeAttachment.dataset.aiRemoveAttachment), 1);
        renderAIAttachments();
        return;
      }
      const copyMessage = event.target.closest("[data-ai-copy-message]");
      if (copyMessage) {
        const message = aiActiveSession(readAIState()).messages.find((item) => item.id === copyMessage.dataset.aiCopyMessage);
        if (message) navigator.clipboard.writeText(message.text);
        return;
      }
      const regenerate = event.target.closest("[data-ai-regenerate]");
      if (regenerate) {
        const state = readAIState();
        const session = aiActiveSession(state);
        const index = session.messages.findIndex((message) => message.id === regenerate.dataset.aiRegenerate);
        const previousUser = session.messages.slice(0, index).reverse().find((message) => message.role === "user");
        if (previousUser) {
          session.messages = session.messages.slice(0, Math.max(0, index));
          writeAIState(state);
          const form = ai.querySelector("[data-ai-chat-form]");
          if (form) aiRunChat(form, previousUser.text.replace(/\n\[Đính kèm:.*\]$/, ""));
        }
        return;
      }
      ...
      if (sessionButton) {
        const state = readAIState();
        const session = state.sessions.find((item) => item.id === sessionButton.dataset.aiSession);
        if (session) {
          state.activeSessionId = session.id;
          writeAIState(state);
          renderAIConversation();
          const last = [...session.messages].reverse().find((message) => message.role === "assistant");
          if (last) aiResult(session.title, last.text);
        }
        return;
      }
...
function initCommunityChatV2() {
  ...
  const canUseChat = () => !document.body.classList.contains("auth-locked") && Boolean(window.HHAuthSession?.token?.());
...
function initHomeNeonInteractions() {
  ...
  setMailto(byId("visitorMailto"), "Danh gia moi tu website Nhhoang", [
  ...
  setMailto(byId("leadMailto"), "Lien he moi tu website Nhhoang", [
...
function initAppShell() {
  ...
  const cosmicLoaderFrom = byId("appCosmicLoaderFrom");
  const cosmicLoaderTo = byId("appCosmicLoaderTo");
  ...
  const syncSidebarToggleState = (expanded = !document.body.classList.contains("app-sidebar-collapsed")) => {
    document.querySelectorAll("[data-shell-toggle]").forEach((button) => {
      button.setAttribute("aria-expanded", String(expanded));
      button.setAttribute("aria-label", expanded ? "Thu gọn thanh điều hướng" : "Mở thanh điều hướng");
    });
  };
  ...
  const davinciResolvePages = [
    { id: "cosmic", icon: "HC", title: "H Cosmic Studio", route: "/davinci-resolve/cosmic", description: "Điều khiển batch production r38 trên web: preflight, timeline V1/V2, effect, RapidGrade, render queue, checkpoint, FFprobe và tải Portable ZIP." },
  ...
  const groups = [
    ...
    { id: "discord", label: "Discord", icon: "DC", accent: "#7c76ff", route: "/discord", items: [] },
    {
      id: "music-ai",
      label: "Làm nhạc AI",
      icon: "♫",
    ...
    },
    {
      ...
      pages: [
        ...
        { id: "lineage", title: "Dòng gene", route: "/game/lineage", description: "Theo dõi thế hệ, đặc tính di truyền, tổ và những khám phá được giữ qua nhiều vòng đời." },
        { id: "observer", title: "Observer & Replay", route: "/game/observer", description: "Quan sát Biomass Ledger, heatmap, dấu vết và phát lại hành trình local gần nhất." },
        { id: "network", title: "Multiplayer Readiness", route: "/game/network", description: "Kiểm tra các điều kiện bảo mật trước multiplayer; không hiển thị phòng hay người online giả." },
    ...
    { id: "chinese", label: "HH Chinese", icon: "中", accent: "#ffcf68", route: "/chinese", items: [] },
    {
      ...
      pages: [
        ...
        { id: "scriptures", title: "Canonical Reader", route: "/phat-phap/scriptures", icon: "經" },
        { id: "research", title: "Tìm kiếm học thuật", route: "/phat-phap/research", icon: "索" },
        { id: "scholar", title: "Scholar & OCR Lab", route: "/phat-phap/scholar", icon: "學" },
        { id: "glossary", title: "Từ điển Phật học", route: "/phat-phap/glossary", icon: "字" },
        { id: "map", title: "Bản đồ giáo pháp", route: "/phat-phap/map", icon: "圖" },
        { id: "provenance", title: "Editorial & Trust Center", route: "/phat-phap/provenance", icon: "證" },
        { id: "review", title: "Ôn giáo lý", route: "/phat-phap/review", icon: "習" },
        { id: "practice", title: "Thiền đường số", route: "/phat-phap/practice", icon: "禪" },
        { id: "rituals", title: "Nghi thức căn bản", route: "/phat-phap/rituals", icon: "礼" },
        { id: "chanting", title: "Phòng tụng niệm", route: "/phat-phap/chanting", icon: "誦" },
        { id: "audio", title: "Thư viện nghe", route: "/phat-phap/audio", icon: "聽" },
        { id: "schedule", title: "Lịch tu học", route: "/phat-phap/schedule", icon: "曆" },
        { id: "temple", title: "Trung tâm Phật sự Việt Nam", route: "/phat-phap/temple", icon: "寺" },
        { id: "vietnam", title: "Phật giáo Việt Nam", route: "/phat-phap/vietnam", icon: "越" },
        { id: "talks", title: "Pháp thoại Observatory", route: "/phat-phap/talks", icon: "聽" },
        { id: "request", title: "Thỉnh kinh", route: "/phat-phap/request", icon: "請" },
        { id: "circles", title: "Nhóm đọc riêng tư", route: "/phat-phap/circles", icon: "眾" },
        { id: "qna", title: "HH Phật học có nguồn", route: "/phat-phap/qna", icon: "問" },
        { id: "journal", title: "Nhật ký mã hóa", route: "/phat-phap/journal", icon: "記" },
        { id: "profile", title: "Hồ sơ tu học", route: "/phat-phap/profile", icon: "行" },
        { id: "accessibility", title: "Trợ năng", route: "/phat-phap/accessibility", icon: "輔" },
  ...
  const sidebarSearchAliases = Object.freeze({
    "chat-ai": "ai trợ lý hỏi đáp nghiên cứu viết code ảnh pdf intelligence",
    create: "sáng tạo nội dung ảnh video chiến dịch creator creative",
    draw: "vẽ tranh ảnh canvas brush bút màu silk",
    "music-ai": "nhạc âm thanh audio beat midi phối khí",
    "comic-motion": "truyện tranh animation chuyển động comic",
    "media-design": "ảnh video audio tài liệu thiết kế media design",
    "graphic-design": "đồ họa vector logo typography mockup thiết kế ảnh",
    google: "tìm kiếm web ảnh tin tức tài liệu search",
    "youtube-main": "video xem clip player playlist",
    discord: "cộng đồng server room kênh bot chat",
    communication: "giao tiếp nhắn tin messenger call cộng đồng",
    remote: "điều khiển từ xa chia sẻ màn hình điện thoại máy tính webrtc",
    ...
    dev: "lập trình code api git database developer",
    ...
    "phat-phap": "phật pháp phat phap giáo lý kinh điển canonical reader scholar ocr facsimile work version segment tìm kiếm học thuật study lab từ điển pali sanskrit hán việt thiền tụng niệm nghi thức bản đồ giáo pháp chùa phật giáo việt nam pháp thoại thỉnh kinh tu học đời sống kiểm chứng nguồn lịch thời khóa hồ sơ tu học nhóm đọc riêng tư trợ năng ôn giáo lý thư viện nghe tủ dữ liệu sao lưu khôi phục",
  ...
  const navigationItemForAnyRoute = (route) => visibleNavigationSections().flatMap((section) => section.items).find((item) => navigationItemMatchesRoute(item, route));
  ...
  const readSidebarRecent = () => readStoredArray(sidebarStorageKey("recent.v1")).filter((route) => navigationItemForRoute(route)).slice(0, 8);
  const rememberSidebarRoute = (route) => {
    const item = navigationItemForAnyRoute(route);
    if (!item) return;
    const recent = readSidebarRecent();
    localStorage.setItem(sidebarStorageKey("recent.v1"), JSON.stringify([item.route, ...recent.filter((entry) => entry !== item.route)].slice(0, 8)));
  };
  ...
  const readSidebarItemOrder = () => {
    try { const value = JSON.parse(localStorage.getItem(sidebarStorageKey("item-order.v1")) || "{}"); return value && typeof value === "object" && !Array.isArray(value) ? value : {}; } catch { return {}; }
  };
  const orderedSidebarItems = (section) => {
    const order = readSidebarItemOrder()[section.id] || [];
    return [...section.items].sort((left, right) => {
      const leftIndex = order.indexOf(left.route), rightIndex = order.indexOf(right.route);
      return (leftIndex < 0 ? Number.MAX_SAFE_INTEGER : leftIndex) - (rightIndex < 0 ? Number.MAX_SAFE_INTEGER : rightIndex);
    });
  };
  const saveSidebarItemOrder = (sectionId, routes) => {
    const state = readSidebarItemOrder();
    state[sectionId] = routes;
    localStorage.setItem(sidebarStorageKey("item-order.v1"), JSON.stringify(state));
  };
  let pinnedSectionCollapsed = localStorage.getItem(sidebarStorageKey("pins-collapsed.v1")) === "true";
  ...
  const renderNavigation = () => {
    ...
    const pinnedMarkup = pinnedItems.length ? `<section class="app-sidebar__pinned ${pinnedSectionCollapsed ? "is-collapsed" : ""}" data-sidebar-pinned-section>
  ...
  const applySidebarSearch = (value = "") => {
    ...
    const accentSensitive = rawQuery !== normalizeSidebarSearch(rawQuery);
    const query = accentSensitive ? rawQuery : normalizeSidebarSearch(rawQuery);
    const queryTokens = query.split(/[^\p{L}\p{N}]+/u).filter(Boolean);
    const matchesSidebarSearch = (searchable = "") => {
      const source = accentSensitive ? String(searchable).toLowerCase() : normalizeSidebarSearch(searchable);
    ...
    navigation.querySelectorAll("[data-nav-section]").forEach((section) => {
      const sectionMatch = Boolean(query) && matchesSidebarSearch(section.dataset.navSearchLabel);
      let sectionMatches = 0;
      section.querySelectorAll(":scope > .app-sidebar__submenu > [data-nav-search-text]").forEach((row) => {
        const visible = !query || sectionMatch || matchesSidebarSearch(row.dataset.navSearchText);
        row.hidden = !visible;
    ...
    const homeMatch = Boolean(query) && matchesSidebarSearch("trang chủ home tổng quan");
    if (home) home.hidden = Boolean(query) && !homeMatch;
    if (homeMatch) matches += 1;
    const recentSection = navigation.querySelector(".app-sidebar__recent");
  ...
  navigation.addEventListener("keydown", (event) => {
    const toolRow = event.target.closest("[data-sidebar-tool-route]");
    if (toolRow && (event.key === "ContextMenu" || (event.shiftKey && event.key === "F10"))) {
      event.preventDefault();
      openSidebarContextMenu(event, toolRow);
      return;
    }
  ...
  navigation.addEventListener("dragstart", (event) => {
    if (event.target.closest(".app-sidebar__flyout")) { event.preventDefault(); return; }
    const pinned = event.target.closest("[data-pinned-route]");
    const item = event.target.closest("[data-nav-item-route]");
    const section = event.target.closest("[data-nav-section]");
    let dragNode = null;
    if (pinned) { sidebarDrag = { type: "pin", id: pinned.dataset.pinnedRoute }; dragNode = pinned; }
    else if (item) { sidebarDrag = { type: "item", id: item.dataset.navItemRoute, sectionId: item.dataset.sidebarSectionId }; dragNode = item; }
    else if (section) { sidebarDrag = { type: "section", id: section.dataset.navSection }; dragNode = section; }
    else return;
    event.dataTransfer.effectAllowed = "move";
    event.dataTransfer.setData("text/plain", sidebarDrag.id);
    requestAnimationFrame(() => dragNode?.classList.add("is-dragging"));
  });
  navigation.addEventListener("dragover", (event) => {
    if (!sidebarDrag) return;
    const target = sidebarDrag.type === "pin" ? event.target.closest("[data-pinned-route]") : sidebarDrag.type === "item" ? event.target.closest("[data-nav-item-route]") : event.target.closest("[data-nav-section]");
    if (sidebarDrag.type === "item" && target?.dataset.sidebarSectionId !== sidebarDrag.sectionId) return;
    if (!target) return;
    event.preventDefault();
  ...
  navigation.addEventListener("drop", (event) => {
    ...
    if (!target) return;
    event.preventDefault();
    const targetId = sidebarDrag.type === "pin" ? target.dataset.pinnedRoute : sidebarDrag.type === "item" ? target.dataset.navItemRoute : target.dataset.navSection;
    const order = sidebarDrag.type === "pin"
      ? readSidebarPins()
      : sidebarDrag.type === "item"
        ? [...target.closest(".app-sidebar__submenu").querySelectorAll(":scope > [data-nav-item-route]")].map((row) => row.dataset.navItemRoute)
    ...
    if (fromIndex >= 0 && targetIndex >= 0 && fromIndex !== targetIndex) {
      ...
      if (sidebarDrag.type === "pin") saveSidebarPins(order);
      else if (sidebarDrag.type === "item") saveSidebarItemOrder(sidebarDrag.sectionId, order);
      else saveSidebarSectionOrder(order);
    }
    sidebarDrag = null;
    renderNavigation();
  ...
  sidebarContextMenu.className = "app-sidebar-context-menu";
  sidebarContextMenu.hidden = true;
  sidebarContextMenu.setAttribute("role", "menu");
  shell.append(sidebarContextMenu);
  const closeSidebarContextMenu = () => {
    sidebarContextMenu.hidden = true;
    sidebarContextMenu.innerHTML = "";
    sidebarContextMenu.removeAttribute("data-route");
  };
  const openSidebarContextMenu = (event, row) => {
    const route = row.dataset.sidebarToolRoute;
    const item = navigationItemForRoute(route);
    if (!item) return;
    const pinned = readSidebarPins().includes(route);
    const favorite = readSidebarFavorites().includes(route);
    sidebarContextMenu.dataset.route = route;
    sidebarContextMenu.innerHTML = `<header><span>${safeText(item.icon)}</span><div><small>TÙY CHỌN CÔNG CỤ</small><strong>${safeText(item.label)}</strong></div></header>
      <button type="button" role="menuitem" data-sidebar-menu-action="pin"><span>${pinned ? "★" : "☆"}</span>${pinned ? "Bỏ ghim" : "Ghim lên đầu"}</button>
      <button type="button" role="menuitem" data-sidebar-menu-action="favorite"><span>${favorite ? "◆" : "◇"}</span>${favorite ? "Bỏ yêu thích" : "Thêm yêu thích"}</button>
      <button type="button" role="menuitem" data-sidebar-menu-action="new-tab"><span>↗</span>Mở trong tab mới</button>
      <button type="button" role="menuitem" data-sidebar-menu-action="hide"><span>◌</span>Ẩn khỏi sidebar</button>`;
    sidebarContextMenu.hidden = false;
    const sourceRect = row.getBoundingClientRect();
    const anchorX = Number.isFinite(event.clientX) && event.clientX > 0 ? event.clientX : sourceRect.right - 8;
    const anchorY = Number.isFinite(event.clientY) && event.clientY > 0 ? event.clientY : sourceRect.top + 12;
    const menuWidth = 226, menuHeight = 218;
    sidebarContextMenu.style.left = `${Math.max(8, Math.min(innerWidth - menuWidth - 8, anchorX))}px`;
    sidebarContextMenu.style.top = `${Math.max(8, Math.min(innerHeight - menuHeight - 8, anchorY))}px`;
    requestAnimationFrame(() => sidebarContextMenu.querySelector("[role=menuitem]")?.focus({ preventScroll: true }));
  };
  navigation.addEventListener("contextmenu", (event) => {
    const row = event.target.closest("[data-sidebar-tool-route]");
    if (!row) return;
    event.preventDefault();
    openSidebarContextMenu(event, row);
  });
  sidebarContextMenu.addEventListener("click", (event) => {
    ...
    const route = sidebarContextMenu.dataset.route;
    if (!action || !route) return;
    if (action === "pin") {
      const pins = readSidebarPins(), removing = pins.includes(route);
      if (!removing && pins.length >= 5) setSidebarStatus("Bạn chỉ có thể ghim tối đa 5 công cụ.");
      else saveSidebarPins(removing ? pins.filter((entry) => entry !== route) : [...pins, route]);
    } else if (action === "favorite") {
      const favorites = readSidebarFavorites();
      saveSidebarFavorites(favorites.includes(route) ? favorites.filter((entry) => entry !== route) : [...favorites, route]);
    } else if (action === "new-tab") {
      const url = new URL(location.href);
      url.hash = `#${route}`;
      window.open(url.href, "_blank", "noopener,noreferrer");
    } else if (action === "hide") {
      saveHiddenSidebarRoutes([...readHiddenSidebarRoutes(), route]);
      saveSidebarPins(readSidebarPins().filter((entry) => entry !== route));
    }
    closeSidebarContextMenu();
    if (action !== "new-tab") renderNavigation();
    setSidebarStatus(action === "hide" ? "Đã ẩn công cụ. Bạn có thể khôi phục bên dưới ô tìm kiếm." : "Đã cập nhật công cụ.");
  });
  const animateSidebarPin = (source, removing = false) => {
    if (!source || matchMedia("(prefers-reduced-motion: reduce)").matches) return;
    const rect = source.getBoundingClientRect();
    const ghost = document.createElement("span");
    ghost.className = `app-sidebar-pin-flight ${removing ? "is-removing" : ""}`;
    ghost.textContent = "★";
  ...
  const sidebarElement = navigation.closest(".app-sidebar");
  let sidebarSheetGesture = null;
  sidebarElement?.addEventListener("pointerdown", (event) => {
    if (!mobileSidebarQuery.matches || !event.target.closest(".app-sidebar__mobile-title") || event.target.closest("button")) return;
    sidebarSheetGesture = { pointerId: event.pointerId, startY: event.clientY, distance: 0 };
    sidebarElement.setPointerCapture?.(event.pointerId);
    sidebarElement.classList.add("is-sheet-dragging");
  });
  sidebarElement?.addEventListener("pointermove", (event) => {
    if (!sidebarSheetGesture || sidebarSheetGesture.pointerId !== event.pointerId) return;
    sidebarSheetGesture.distance = Math.max(0, event.clientY - sidebarSheetGesture.startY);
    sidebarElement.style.setProperty("--sheet-drag-y", `${sidebarSheetGesture.distance}px`);
  });
  const finishSidebarSheetGesture = (event) => {
    if (!sidebarSheetGesture || sidebarSheetGesture.pointerId !== event.pointerId) return;
    const shouldClose = sidebarSheetGesture.distance > 84;
    sidebarSheetGesture = null;
    sidebarElement.classList.remove("is-sheet-dragging");
    sidebarElement.style.removeProperty("--sheet-drag-y");
    if (shouldClose) {
      document.body.classList.add("app-sidebar-collapsed");
      syncMobileSidebarDock();
    }
  };
  sidebarElement?.addEventListener("pointerup", finishSidebarSheetGesture);
  sidebarElement?.addEventListener("pointercancel", finishSidebarSheetGesture);
  document.addEventListener("pointerdown", (event) => {
    if (!event.target.closest(".app-sidebar-context-menu")) closeSidebarContextMenu();
  });
  ...
  const cosmicLoaderMessages = {
    ...
    "davinci-resolve": "Đang tải Media Pool, timeline và bộ kết xuất video trên web...",
    ...
    discord: "Đang kết nối Discord OAuth, server và kênh cộng đồng...",
  ...
  const cosmicMotionMode = () => {
    if (matchMedia("(prefers-reduced-motion: reduce)").matches || document.body.classList.contains("app-reduce-motion") || document.body.dataset.appEffects === "off") return "static";
    return document.body.dataset.appEffects === "full" ? "cinematic" : "balanced";
  };
  ...
  const hideCosmicRouteLoaderImmediately = (outcome = "cancelled") => {
    if (!cosmicRouteLoader) return;
    const startedAt = Number(cosmicRouteLoader.dataset.transitionStartedAt || 0);
    if (startedAt) {
      cosmicRouteLoader.dataset.lastTransitionDuration = String(Math.max(0, Date.now() - startedAt));
      cosmicRouteLoader.dataset.lastTransitionKind = cosmicRouteLoader.dataset.transitionKind || "home";
      cosmicRouteLoader.dataset.lastTransitionOutcome = outcome;
      delete cosmicRouteLoader.dataset.transitionStartedAt;
      window.dispatchEvent(new CustomEvent(outcome === "complete" ? "hh:route-transition-complete" : "hh:route-transition-cancel", { detail: { route: cosmicLoaderRoute, kind: cosmicRouteLoader.dataset.lastTransitionKind, outcome } }));
    }
    cosmicRouteLoader.dataset.transitionState = "idle";
    cosmicLoaderRun += 1;
  ...
  const showCosmicRouteLoader = (route = routeFromHash()) => {
    if (!cosmicRouteLoader) return;
    const normalized = String(route || "/home").split("?")[0];
    if (document.body.classList.contains("hh-surface-pending") || document.body.classList.contains("hh-surface-releasing")) {
      hideCosmicRouteLoaderImmediately("superseded");
      return;
    }
    ...
    cosmicRouteLoader.classList.add("is-departing");
    cosmicRouteLoader.dataset.transitionKind = meta.kind;
    cosmicRouteLoader.dataset.motion = motion;
    cosmicRouteLoader.dataset.transitionState = "departing";
    cosmicRouteLoader.dataset.transitionStartedAt = String(Date.now());
    cosmicRouteLoader.style.setProperty("--loader-accent", meta.accent);
    cosmicRouteLoader.style.setProperty("--loader-secondary", meta.secondary);
    cosmicRouteLoader.style.setProperty("--transit-duration", motion === "cinematic" ? "520ms" : motion === "balanced" ? "340ms" : "40ms");
    ...
    if (cosmicLoaderFrom) cosmicLoaderFrom.textContent = continuing ? cosmicLoaderFrom.textContent : previousTitle;
    if (cosmicLoaderTo) cosmicLoaderTo.textContent = meta.title;
    window.dispatchEvent(new CustomEvent("hh:route-transition-start", { detail: { route: normalized, kind: meta.kind, motion } }));
    if (!continuing) {
    ...
    cosmicLoaderPhaseTimers.push(
      ...
      // the user. The route placeholder/error remains available underneath.
      setTimeout(() => {
        if (run !== cosmicLoaderRun) return;
        hideCosmicRouteLoaderImmediately("timeout");
  ...
  const finishCosmicRouteLoader = ({ error = false, message = "" } = {}) => {
    ...
    if (document.body.classList.contains("hh-surface-pending") || document.body.classList.contains("hh-surface-releasing")) {
      hideCosmicRouteLoaderImmediately("singularity");
      return;
    }
    const run = cosmicLoaderRun;
    const motion = cosmicRouteLoader.dataset.motion || "balanced";
    clearCosmicLoaderTimers();
    ...
    cosmicLoaderHideTimer = window.setTimeout(() => {
      if (run === cosmicLoaderRun) hideCosmicRouteLoaderImmediately(error ? "error" : "complete");
    }, duration);
  };
  ...
  const renderRoute = () => {
    ...
    document.body.classList.toggle("app-cinema-route", route === "/cinema" || route.startsWith("/cinema/"));
    document.body.classList.toggle("app-music-library-route", route === "/music" || route.startsWith("/music/"));
    ...
    document.body.classList.toggle("app-discord-route", route === "/discord");
    ...
    if (route !== "/cinema" && !route.startsWith("/cinema/")) window.HHCinemaHub?.unmount?.();
    if (route !== "/music" && !route.startsWith("/music/")) window.HHMusicLibrary?.unmount?.();
    ...
    if (route !== "/discord") window.HHDiscordHub?.unmount?.();
    ...
    if (route !== "/social-media-tools" && !route.startsWith("/social-media-tools/")) window.HHSocialMediaTools?.unmount?.();
    setUser();
    renderNavigation();
    requestAnimationFrame(() => {
    ...
    if (routePath === "/platform") {
      ...
      updatePageHeader("Discord", "Liên kết tài khoản, mở server và trò chuyện an toàn bằng HH Discord Bot trong workspace riêng.", route);
      pageActions.innerHTML = `<a href="https://discord.com/app" target="_blank" rel="noopener">Mở Discord ↗</a><button class="app-primary-action" type="button" data-discord-workspace-focus>⌕ Tìm kiếm</button>`;
      workspace.innerHTML = '<div data-discord-hub-host></div>';
      const discordHost = workspace.firstElementChild;
      const mounted = window.HHDiscordHub?.mount?.(discordHost, { apiBase: String(window.HH_API_BASE || REALTIME_URL || location.origin) });
      if (!mounted) mountSimpleView("Discord", "Không thể khởi tạo Discord Center.", '<button type="button" data-shell-retry-route>Thử lại</button>');
      pageActions.querySelector("[data-discord-workspace-focus]")?.addEventListener("click", () => window.HHDiscordHub?.focus?.());
    ...
    } else if (route === "/copyright" || route.startsWith("/copyright/")) {
      updatePageHeader("Bản quyền & giấy phép", "Kiểm tra quyền sử dụng, hồ sơ nguồn và gửi khiếu nại cho nội dung Phim hoặc Nhạc trên hoang8.com.", route);
      pageActions.innerHTML = `<button type="button" data-app-route="/cinema">Mở Phim</button><button type="button" data-app-route="/music">Mở Nhạc</button><button class="app-primary-action" type="button" data-copyright-email>Email bản quyền</button>`;
      workspace.innerHTML = '<div data-open-media-governance-host></div>';
      if (window.HHOpenMediaGovernance?.mount) window.HHOpenMediaGovernance.mount(workspace.firstElementChild, {
        apiBase: REALTIME_URL,
        currentUser: readCurrentAuthUser(),
        route,
        contactEmail: "nhhoang130803@gmail.com",
        navigate: (nextRoute) => { location.hash = `#${nextRoute}`; }
      });
      else mountSimpleView("Bản quyền & giấy phép", "Đang tải trung tâm quyền nội dung...", "");
    } else if (route === "/cinema" || route.startsWith("/cinema/")) {
      updatePageHeader("Phim", "Xem phim công cộng và phim có giấy phép mở, kèm nguồn, tác giả và điều kiện sử dụng minh bạch.", route);
      pageActions.innerHTML = `<button type="button" data-app-route="/music">Mở Nhạc</button><button type="button" data-app-route="/copyright">Quyền & giấy phép</button><button class="app-primary-action" type="button" data-cinema-search-focus>Tìm phim</button>`;
      workspace.innerHTML = '<div data-cinema-hub-host></div>';
      if (window.HHCinemaHub?.mount) window.HHCinemaHub.mount(workspace.firstElementChild, {
        apiBase: REALTIME_URL,
        currentUser: readCurrentAuthUser(),
        route,
        navigate: (nextRoute) => { location.hash = `#${nextRoute}`; }
      });
      else mountSimpleView("Phim", "Đang tải thư viện phim có giấy phép mở...", "");
    } else if (route === "/music" || route.startsWith("/music/")) {
      updatePageHeader("Nhạc", "Nghe nhạc Public Domain và Creative Commons được phép, với thông tin nguồn và giấy phép cho từng bản ghi.", route);
      pageActions.innerHTML = `<button type="button" data-app-route="/cinema">Mở Phim</button><button type="button" data-app-route="/copyright">Quyền & giấy phép</button><button class="app-primary-action" type="button" data-music-search-focus>Tìm nhạc</button>`;
      workspace.innerHTML = '<div data-open-music-host></div>';
      if (window.HHMusicLibrary?.mount) window.HHMusicLibrary.mount(workspace.firstElementChild, {
        apiBase: REALTIME_URL,
        currentUser: readCurrentAuthUser(),
        route
      });
      ...
      updatePageHeader("Phòng học tập trung", "Trung tâm Deep Focus với 230 không gian (30 cảnh gốc + Atlas 200), bạn đồng hành 3D, 16 âm môi trường, Pomodoro và dữ liệu học thật.", route);
      ...
      if (!mounted) mountSimpleView("Phòng học tập trung", "Không thể khởi tạo Focus Room.", '<button type="button" data-shell-retry-route>Thử lại</button>');
      ...
      updatePageHeader(learningMeta?.title || "HH School", "Trường học số lớp 1–12 theo Chương trình GDPT 2018, học bằng nội dung nguyên bản và bằng chứng kỹ năng.", route);
      pageActions.innerHTML = `<button type="button" data-app-route="/learn/practice">Luyện tập</button><button class="app-primary-action" type="button" data-app-route="/learn/today">Hôm nay</button>`;
      workspace.innerHTML = '<div data-hh-school-shell></div>';
      if (window.HHSchool?.mount) window.HHSchool.mount(workspace.firstElementChild, {
      ...
      updatePageHeader("HH English", "Trung tâm học thuật theo CEFR 2020 với 19 chế độ luyện, cấp A0–C2 và 70 lộ trình nghề nghiệp tự thích ứng.", route);
    ...
    } else if (route === "/japanese" || route.startsWith("/japanese/")) {
      updatePageHeader("HH Japanese OS V5", "42.301 từ/cụm, 30.000 câu có nguồn, 800 nhiệm vụ, Active Vocabulary, Particle Lab, Smart Reader V3 và Kanji Writing V2.", route);
      workspace.innerHTML = '<div data-hh-japanese-host></div>';
      if (window.HHJapanese?.mount) window.HHJapanese.mount(workspace.firstElementChild, {
      ...
      if (window.HHDrawStudio?.mount) {
        window.HHDrawStudio.mount(workspace.firstElementChild, { currentUser: readCurrentAuthUser() });
      ...
      remember("draw");
    } else if (route === "/remote" || route.startsWith("/remote/")) {
      updatePageHeader("HH Quantum Remote", "Hỗ trợ từ xa WebRTC với PIN một lần, phê duyệt thiết bị, quyền theo tác vụ, chat, con trỏ, tệp P2P, ghi hình và chất lượng tự thích ứng.", route);
      pageActions.innerHTML = `<button type="button" data-remote-global-assist>Hỗ trợ trực tiếp</button><button type="button" data-remote-global-security>Bảo mật</button><button class="app-primary-action" type="button" data-remote-global-connect>Kết nối thiết bị</button>`;
      workspace.innerHTML = '<div data-remote-hub-host></div>';
      if (window.HHRemoteHub?.mount) {
        const remoteHost = workspace.firstElementChild;
        window.HHRemoteHub.mount(remoteHost, {
          currentUser: readCurrentAuthUser(),
          socketUrl: SOCKET_URL,
          apiBase: String(window.HH_API_BASE || location.origin)
        });
        pageActions.querySelector("[data-remote-global-assist]")?.addEventListener("click", () => remoteHost.querySelector('[data-remote-view="assist"]')?.click());
        pageActions.querySelector("[data-remote-global-security]")?.addEventListener("click", () => remoteHost.querySelector('[data-remote-view="security"]')?.click());
      ...
      updatePageHeader("HH Intelligence", "Trợ lý đa phương thức của HH: trò chuyện nhiều lượt, nghiên cứu có nguồn, phân tích ảnh/PDF, viết và lập trình.", route);
      pageActions.innerHTML = `<button type="button" data-app-route="/create/ai-center">Công cụ AI</button><button class="app-primary-action" type="button" data-app-route="/chat-ai/new">＋ Chat mới</button>`;
      ...
      else if (resolveView === "ai-video-remake") mountSimpleView("AI Video Remake Studio", "Đang tải không gian tạo và remix video AI...", "");
  ...
  const renderRouteWithTransition = () => {
    ...
    if (renderedRoute && nextRoute === renderedRoute && !pendingAssetRoute) {
      renderRouteSafely();
      return;
    }
  ...
  const searchItems = () => {
    ...
    modules.unshift(
      ...
      },
      {
        ...
        route: "/remote",
        key: "remote máy tính điện thoại chia sẻ màn hình screen share webrtc p2p pin thiết bị chat con trỏ clipboard truyền tệp hỗ trợ từ xa"
      },
      {
        type: "Bản quyền",
        title: "Bản quyền & giấy phép",
        description: "Tra cứu quyền sử dụng, tải hồ sơ ghi công và gửi khiếu nại nội dung tới hoang8.com.",
        route: "/copyright",
        key: "bản quyền copyright giấy phép license rights attribution takedown khiếu nại phim nhạc content id"
      },
      {
        type: "Mạng xã hội",
  ...
  const executePaletteOption = (option) => {
    ...
    if (action.startsWith("project-view:")) {
      ...
      if (location.hash === nextHash) renderRouteWithTransition();
      else {
  ...
  document.addEventListener("click", (event) => {
    ...
    if (event.target.closest("[data-sidebar-pins-toggle]")) {
      pinnedSectionCollapsed = !pinnedSectionCollapsed;
      localStorage.setItem(sidebarStorageKey("pins-collapsed.v1"), String(pinnedSectionCollapsed));
      renderNavigation();
      return;
    }
    if (event.target.closest("[data-sidebar-restore-hidden]")) {
      saveHiddenSidebarRoutes([]);
      renderNavigation();
      setSidebarStatus("Đã khôi phục toàn bộ công cụ bị ẩn.");
      return;
    }
    ...
    if (event.target.closest("[data-cinema-search-focus]")) {
      window.HHCinemaHub?.focusSearch?.();
      document.querySelector("[data-cinema-search]")?.focus({ preventScroll: true });
      return;
    }
    if (event.target.closest("[data-music-search-focus]")) {
      window.HHMusicLibrary?.focusSearch?.();
      document.querySelector("[data-omh-search]")?.focus({ preventScroll: true });
      return;
    }
    ...
    if (sidebarPin) {
      ...
      animateSidebarPin(sidebarPin, removing);
  ...
  document.addEventListener("change", (event) => {
    ...
    saveState(next);
    if (setting.dataset.shellSetting === "collapsed") {
      document.body.classList.toggle("app-sidebar-collapsed", setting.checked);
      syncMobileSidebarDock();
    }
