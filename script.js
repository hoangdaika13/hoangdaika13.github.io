...
function initSuperPlatform() {
  ...
  const loadCommunityFeed=async({silent=false}={})=>{const panel=grid.querySelector("[data-community-center]");if(!panel)return;const status=panel.querySelector("[data-community-status]");try{const data=await communityApi();const state=ensureCommunityState();state.remotePosts=data.posts||[];state.lastSync=Date.now();writeCommunityState(state);const list=panel.querySelector("[data-community-posts]");if(list)list.innerHTML=(state.remotePosts.length?state.remotePosts:communityDefaults()).map(communityPostMarkup).join("");if(status)status.textContent=`Đã đồng bộ ${state.remotePosts.length} bài viết · ${new Date().toLocaleTimeString("vi-VN")}`;}catch(error){if(status)status.textContent=`Bản tin dự phòng · ${error.message}`;}if(!silent){clearInterval(communityLoadTimer);communityLoadTimer=setInterval(()=>{if(grid.querySelector("[data-community-center]"))loadCommunityFeed({silent:true});else clearInterval(communityLoadTimer);},30000);}};
  const communityMutate=async(body)=>{const data=await communityApi({method:"POST",body});await loadCommunityFeed({silent:true});return data;};
  ...
  grid.addEventListener("click", (event) => {
    ...
    const community=event.target.closest("[data-community-center]");if(community){
      const react=event.target.closest("[data-post-react]");if(react){const type=react.dataset.currentReaction?"remove":"like";communityMutate({action:"react",postId:react.dataset.postReact,type}).catch((error)=>window.alert(error.message));return;}
      const picked=event.target.closest("[data-post-reaction]");if(picked){communityMutate({action:"react",postId:picked.dataset.postReaction,type:picked.dataset.reactionType}).catch((error)=>window.alert(error.message));return;}
      const save=event.target.closest("[data-post-save]");if(save){communityMutate({action:"save",postId:save.dataset.postSave}).catch((error)=>window.alert(error.message));return;}
      ...
      if(event.target.closest("[data-community-add-media]")){const fields=community.querySelector("[data-community-media-fields]");fields.hidden=!fields.hidden;if(!fields.hidden)fields.querySelector("input")?.focus();return;}
      ...
      const filter=event.target.closest("[data-community-filter]");if(filter){community.querySelectorAll("[data-community-filter]").forEach((item)=>item.classList.toggle("active",item===filter));const state=ensureCommunityState();community.querySelectorAll("[data-post-id]").forEach((node)=>{const post=(state.remotePosts||state.posts||[]).find((item)=>String(item.id)===node.dataset.postId);node.hidden=filter.dataset.communityFilter==="popular"&&(post?.reactionCount||post?.likes||0)<5||filter.dataset.communityFilter==="saved"&&!post?.saved;});return;}
      const view=event.target.closest("[data-community-view]");if(view){const status=community.querySelector("[data-community-status]");const mode=view.dataset.communityView;if(mode==="creator"){location.hash="/create/creator-studio";return;}const messages={friends:"Gợi ý kết nối đang hiển thị ở cột phải. Theo dõi một thành viên để thêm họ vào bảng tin.",groups:"Nhóm của bạn sẽ xuất hiện khi cộng đồng có thêm thành viên và phòng chung.",events:"Sự kiện cộng đồng đang được lọc theo chủ đề Thông báo.",memories:"Kỷ niệm sẽ tổng hợp lại bài viết cũ của chính bạn."};if(status)status.textContent=messages[mode]||"Đã chuyển chế độ xem.";if(mode==="events")community.querySelector('[data-community-topic="Thông báo"]')?.click();if(mode==="memories"){const list=community.querySelector("[data-community-posts]");if(list)[...list.children].reverse().forEach((post)=>list.append(post));}return;}
      const follow=event.target.closest("[data-community-follow]");if(follow){follow.classList.toggle("active");follow.textContent=follow.classList.contains("active")?"Đang theo dõi":"Theo dõi";return;}
      const contact=event.target.closest("[data-community-contact]");if(contact){location.hash="community";return;}
      const more=event.target.closest("[data-post-more]");if(more){const action=window.prompt("Tác vụ: nhập 'xóa' để xóa bài của bạn hoặc 'báo cáo' để đánh dấu.")?.trim().toLowerCase();if(action==="xóa")communityApi({method:"DELETE",query:`?id=${encodeURIComponent(more.dataset.postMore)}`}).then(()=>loadCommunityFeed({silent:true})).catch((error)=>window.alert(error.message));else if(action==="báo cáo")window.alert("Cảm ơn bạn. Báo cáo đã được ghi nhận để quản trị viên xem xét.");return;}
      if(event.target.closest("[data-story-create]")){const content=window.prompt("Bạn muốn chia sẻ gì trong Tin?")?.trim();if(!content)return;const row=community.querySelector(".community-stories");const story=document.createElement("button");story.className="community-story interactive is-new";story.type="button";story.dataset.story="mine";story.title=content;story.innerHTML=`<span>${communityInitials(communityViewerName())}</span><strong>Tin của bạn</strong>`;row?.querySelector("[data-story-create]")?.after(story);const status=community.querySelector("[data-community-status]");if(status)status.textContent="Đã tạo Tin trên thiết bị này. Tin sẽ tự ẩn khi tải lại trang.";return;}
    }
