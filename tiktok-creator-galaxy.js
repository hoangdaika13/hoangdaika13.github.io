(function tiktokCreatorGalaxy(global) {
  ...
  const safeTikTokUrl = (value) => { try { const url = new URL(String(value || "")); return url.protocol === "https:" && (url.hostname === "tiktok.com" || url.hostname.endsWith(".tiktok.com")) ? url.toString() : ""; } catch { return ""; } };
  ...
  const formatNumber = (value) => { const number = Number(value || 0); return Number.isFinite(number) ? number.toLocaleString("vi-VN", { maximumFractionDigits: 2 }) : "0"; };
  ...
  function connectionButton() {
    ...
    if (account) return `<button class="ttg-account-pill" type="button" data-action="workspace" data-workspace="developer"><span>${safeHttpsUrl(account.avatarUrl) ? `<img src="${esc(safeHttpsUrl(account.avatarUrl))}" alt="">` : "TT"}</span><b>${esc(account.displayName || account.username || "TikTok")}</b><small>${account.active ? "Đang dùng" : "Đã kết nối"}</small></button>`;
    if (remote.authRequired || core().currentOwnerId() === "guest") return `<button class="ttg-btn ttg-btn--primary" type="button" data-action="route" data-route="/login">Đăng nhập để kết nối</button>`;
  ...
  function topbar() {
    return `<header class="ttg-topbar">
      ...
      <div class="ttg-top-status"><span class="${navigator.onLine ? "is-online" : "is-offline"}">${navigator.onLine ? "Online" : "Offline"}</span><span>${remote.authRequired ? "Đăng nhập để kiểm tra Login Kit" : remote.configured ? "Login Kit sẵn sàng" : "Login Kit chưa cấu hình"}</span><span>${remote.authRequired ? "Trạng thái scope chưa tải" : remote.audited ? "Content Posting đã audit" : "Chưa audit · SELF_ONLY"}</span></div>
  ...
  function importToolbar(type, label = "Nhập snapshot") {
    ...
    return `<div class="ttg-toolbar"><label class="ttg-btn">${label}<input hidden type="file" accept=".csv,.tsv,.json,application/json,text/csv" data-import="${esc(type)}"></label>${rows.length ? `<button class="ttg-btn" type="button" data-action="export-dataset" data-dataset="${esc(type)}" data-format="json">Xuất JSON</button><button class="ttg-btn" type="button" data-action="export-dataset" data-dataset="${esc(type)}" data-format="csv">Xuất CSV</button><label class="ttg-dataset-search"><span>Tìm</span><input type="search" data-dataset-search="${esc(type)}" value="${esc(state.datasetSearch?.[type] || "")}" placeholder="Lọc record…"></label><button class="ttg-btn ttg-btn--danger" type="button" data-action="clear-dataset" data-dataset="${esc(type)}">Xóa dữ liệu cục bộ</button>` : ""}<span>${rows.length.toLocaleString("vi-VN")} record · nguồn người dùng cung cấp</span></div>`;
  }
  function filteredRows(rows, id) {
    const query = String(state.datasetSearch?.[id] || "").trim().toLocaleLowerCase("vi");
    return query ? rows.filter((row) => Object.values(row || {}).some((value) => String(value).toLocaleLowerCase("vi").includes(query))) : rows;
  }
  function genericTable(rows, max = 50, id = state.workspace) {
    if (!rows.length) return ""; const visible = filteredRows(rows, id);
    const keys = [...new Set(rows.slice(0, max).flatMap((row) => Object.keys(row || {})))].slice(0, 8);
    return `${!visible.length ? `<div class="ttg-empty ttg-empty--small"><span>⌕</span><strong>Không có record khớp</strong><p>Đổi từ khóa tìm kiếm để xem lại dữ liệu.</p></div>` : `<div class="ttg-table-wrap"><table><thead><tr><th>#</th>${keys.map((key) => `<th>${esc(key)}</th>`).join("")}</tr></thead><tbody>${visible.slice(0, max).map((row, index) => `<tr><td>${index + 1}</td>${keys.map((key) => `<td title="${esc(row[key])}">${esc(row[key])}</td>`).join("")}</tr>`).join("")}</tbody></table></div>`}${visible.length > max ? `<p class="ttg-table-note">Đang preview ${max}/${visible.length} record khớp. Toàn bộ dữ liệu vẫn được giữ cục bộ.</p>` : ""}`;
  }
  function datasetSummary(id, rows) {
    const summary = analytics().summarizeDataset(id, rows); if (!summary.metrics?.length) return "";
    return `<section class="ttg-dataset-summary"><div class="ttg-metrics">${summary.metrics.map((item) => `<article><small>${esc(item.label)}</small><strong>${formatNumber(item.value)}${esc(item.suffix || "")}</strong><span>Dữ liệu file nhập</span></article>`).join("")}</div>${summary.top?.length ? `<div class="ttg-card"><h3>Top theo cột phù hợp có trong file</h3><div class="ttg-rank-list">${summary.top.map((item, index) => `<p><i>${index + 1}</i><b>${esc(item.name)}</b><span>${formatNumber(item.value)}</span></p>`).join("")}</div></div>` : ""}<p class="ttg-honest">${esc(summary.note || "")}</p></section>`;
  }

  function trendResults(rows) {
    const ranked = analytics().analyzeTrends(rows);
    if (!ranked.length) return `<div class="ttg-empty ttg-empty--small"><span>⌕</span><strong>Không có trend khớp</strong><p>Đổi từ khóa lọc để xem lại snapshot.</p></div>`;
    return `<div class="ttg-metrics"><article><small>Trend đang xem</small><strong>${ranked.length}</strong><span>Dữ liệu cục bộ</span></article><article><small>Điểm cao nhất</small><strong>${ranked[0]?.score || 0}</strong><span>Điểm giải thích được</span></article><article><small>Tăng nhanh</small><strong>${ranked.filter((item) => item.velocity > 20).length}</strong><span>So với snapshot trước</span></article><article><small>Nguồn</small><strong>User</strong><span>Không scrape TikTok</span></article></div><div class="ttg-card"><h3>Trend radar</h3><div class="ttg-trends">${ranked.slice(0, 30).map((item, index) => `<article><b>${index + 1}</b><div><strong>${esc(item.name)}</strong><small>${formatNumber(item.views)} lượt xem · ${item.posts} video · saturation ${item.saturation}%</small></div><span class="${item.velocity >= 0 ? "is-up" : "is-down"}">${item.velocity >= 0 ? "+" : ""}${item.velocity}%</span><em style="--score:${item.score}%"><i></i>${item.score}</em></article>`).join("")}</div></div>`;
  }
  function trendsView() {
    const allRows = dataset("trends"); const rows = filteredRows(allRows, "trends");
    return `${heading()}${importToolbar("trends", "Nhập trend CSV / JSON")}${!allRows.length ? emptyImport("Bắt đầu bằng snapshot hợp lệ", "Cột gợi ý: name, views, previous_views, posts, relevance. Creative Center chỉ được mở bằng liên kết chính thức.", "trends") : `<section data-dataset-table="trends">${trendResults(rows)}</section>`}<div class="ttg-source-note">TikTok Creative Center không có API công khai dùng cho workspace này. <a href="${core().OFFICIAL_LINKS.creativeCenter}" target="_blank" rel="noopener noreferrer">Mở Creative Center ↗</a></div>`;
  }
  ...
  function scriptView() {
    ...
    const text = script?.output || (script ? `${script.hook}\n\n${script.voiceover}\n\n${script.caption}\n${script.cta}` : "");
    return `${heading()}<div class="ttg-split"><form class="ttg-card ttg-form" data-form="script"><h3>AI Script Director</h3><label><span>Chủ đề</span><input name="topic" required maxlength="240" value="${esc(script?.topic || "")}" placeholder="Ví dụ: 3 cách chụp ảnh sản phẩm đẹp"></label><div class="ttg-two"><label><span>Thời lượng</span><select name="duration"><option value="15">15 giây</option><option value="30" selected>30 giây</option><option value="60">60 giây</option><option value="180">180 giây</option></select></label><label><span>Tone</span><select name="tone"><option>Tự nhiên</option><option>Năng động</option><option>Chuyên gia</option><option>Kể chuyện</option><option>Nhẹ nhàng</option></select></label></div><label><span>Khán giả</span><input name="audience" value="người xem Việt Nam" maxlength="180"></label><div class="ttg-toolbar"><button class="ttg-btn ttg-btn--primary" type="submit" name="generator" value="ai">Tạo bằng AI</button><button class="ttg-btn" type="submit" name="generator" value="local">Tạo nhanh offline</button></div><p class="ttg-honest">AI dùng provider đã cấu hình ở backend và tự chuyển sang bản offline nếu provider tạm lỗi. Không bịa dữ liệu xu hướng.</p></form><div class="ttg-card">${script ? `<div class="ttg-script-head"><h3>${esc(script.topic)}</h3><button class="ttg-btn" type="button" data-action="save-script-project">Lưu dự án</button></div><p class="ttg-ai-source">Nguồn: ${esc(script.provider || script.source || "offline")}${script.model ? ` · ${esc(script.model)}` : ""}</p>${script.output ? `<div class="ttg-ai-output">${esc(script.output).replace(/\n/g, "<br>")}</div>` : `<blockquote>${esc(script.hook)}</blockquote><h4>Voice-over</h4><p>${esc(script.voiceover)}</p><h4>Shot list</h4><ol class="ttg-shot-list">${(script.shots || []).map((shot) => `<li>${esc(shot)}</li>`).join("")}</ol><h4>Caption & CTA</h4><p>${esc(script.caption)}<br>${esc(script.cta)}</p>`}<div class="ttg-toolbar"><button class="ttg-btn" type="button" data-action="copy" data-copy="${esc(text)}">Sao chép toàn bộ</button><button class="ttg-btn" type="button" data-action="download-script">Tải TXT</button></div>` : `<div class="ttg-empty ttg-empty--small"><span>✎</span><strong>Chưa có kịch bản</strong><p>Tạo bằng AI hoặc dùng bộ tạo nhanh offline ngay trên thiết bị.</p></div>`}</div></div>`;
  }

  function voiceView() {
    const cues = state.subtitle?.cues || [];
    return `${heading()}<div class="ttg-split"><div class="ttg-card"><h3>Giọng Việt cục bộ</h3><label class="ttg-field"><span>Nội dung nghe thử</span><textarea data-voice-text>Xin chào, đây là bản đọc thử tiếng Việt cho video TikTok của bạn.</textarea></label><div class="ttg-toolbar"><button class="ttg-btn ttg-btn--primary" type="button" data-action="voice-preview">Nghe thử</button><button class="ttg-btn" type="button" data-action="voice-stop">Dừng</button></div><p class="ttg-honest">Dùng Speech Synthesis của thiết bị. Giọng nữ Việt Nam được ưu tiên khi trình duyệt cung cấp; không gọi đây là chấm phát âm.</p></div><div class="ttg-card"><h3>Subtitle SRT / VTT</h3><label class="ttg-dropzone"><input type="file" accept=".srt,.vtt,text/vtt" data-subtitle><span>CC</span><strong>Chọn SRT hoặc VTT</strong><small>Parser, editor và converter chạy hoàn toàn trên thiết bị.</small></label>${cues.length ? `<form class="ttg-subtitle-editor" data-form="subtitle-editor"><label><span>Chỉnh nội dung và timecode SRT</span><textarea name="subtitleText" spellcheck="false">${esc(core().subtitlesToSrt(cues))}</textarea></label><button class="ttg-btn" type="submit">Áp dụng chỉnh sửa</button></form><div class="ttg-subtitle-preview">${cues.slice(0, 10).map((cue) => `<article><time>${cue.start.toFixed(2)} → ${cue.end.toFixed(2)}</time><p>${esc(cue.text)}</p></article>`).join("")}</div><div class="ttg-toolbar"><button class="ttg-btn" type="button" data-action="subtitle-download" data-format="srt">Xuất SRT</button><button class="ttg-btn" type="button" data-action="subtitle-download" data-format="vtt">Xuất VTT</button></div>` : ""}</div></div>`;
  }
  ...
  function schedulerView() {
    ...
    const canUploadDraft = account?.scopes?.includes("video.upload");
    ...
    const mayPrepare = Boolean(account && publishFile && (directMode ? canPublishDirect && creator : canUploadDraft));
    const creatorPanel = account ? `<section class="ttg-creator-info ${creator ? "is-ready" : ""}">
      <header><div><small>QUYỀN XUẤT BẢN · CREATOR INFO</small><strong>${esc(creator?.nickname || account.displayName || account.username || "TikTok Creator")}</strong></div><div class="ttg-toolbar">${!canUploadDraft ? `<button class="ttg-btn" type="button" data-action="connect-draft">Cấp quyền Upload Draft</button>` : ""}${canPublishDirect ? `<button class="ttg-btn" type="button" data-action="creator-info">${creator ? "Làm mới Creator Info" : "Tải Creator Info"}</button>` : `<button class="ttg-btn" type="button" data-action="connect-direct">Cấp quyền Direct Post</button>`}</div></header>
  ...
  async function refresh(silent = true) {
    ...
    } catch (error) { if (error.status === 401) remote = { ...remote, authRequired: true, configured: false, connections: [], jobs: [], audits: [], providers: {}, security: {} }; render(); if (!silent || error.status !== 401) notify(error.message, "error"); }
    finally { busy = ""; render(); }
  }
  ...
  async function importFile(input) {
    ...
    try {
      ...
      state.datasets[type] = rows; state.datasetSearch = { ...(state.datasetSearch || {}), [type]: "" }; save(); render(); notify(`Đã nhập ${rows.length} record vào kho riêng của tài khoản.`);
  ...
  async function sha256(file) { if (!global.crypto?.subtle) return "Không hỗ trợ Web Crypto"; if (file.size > 256 * 1024 ** 2) return "Bỏ qua cục bộ: file lớn hơn 256 MB"; const data = await file.arrayBuffer(); const digest = await global.crypto.subtle.digest("SHA-256", data); return [...new Uint8Array(digest)].map((byte) => byte.toString(16).padStart(2, "0")).join(""); }
  ...
  async function startUpload(jobId) {
    ...
    const bound = preparedUploads.get(jobId); const uploadFile = bound?.file; const uploadMedia = bound?.media;
    if (!uploadFile) return notify("Job này chưa được gắn đúng video trong phiên hiện tại. Hãy tạo lại job từ form preflight.", "error");
    ...
    try {
      const result = await publishing().uploadJob(jobId, uploadFile, {
        signal: uploadController.signal,
        duration: Number(uploadMedia?.duration || 0),
      ...
      uploadRuntime = { ...uploadRuntime, status: result?.job?.status || "processing", percent: result?.job?.progress || 100, detail: result?.job?.error || "TikTok đã nhận và xử lý xong trạng thái hiện tại." }; preparedUploads.delete(jobId); await refresh(true); notify("Upload hoàn tất; trạng thái lấy từ TikTok.");
  ...
  async function onClick(event) {
    ...
    if (action === "connect-draft") { try { setBusy("Đang xin quyền Upload Draft…"); await connections().connect(["user.info.basic", "video.upload"]); } catch (error) { busy = ""; render(); notify(error.message, "error"); } return; }
    ...
    if (action === "export-dataset") { const id = button.dataset.dataset; const format = button.dataset.format || "json"; const rows = filteredRows(dataset(id), id); if (!rows.length) return notify("Không có record để xuất.", "error"); if (format === "csv") core().download(`tiktok-${id}-${Date.now()}.csv`, core().toCsv(rows), "text/csv;charset=utf-8"); else core().download(`tiktok-${id}-${Date.now()}.json`, JSON.stringify(rows, null, 2), "application/json"); notify(`Đã xuất ${rows.length} record ${format.toUpperCase()}.`); return; }
    if (action === "clear-dataset") { const id = button.dataset.dataset; if (global.confirm("Xóa bộ dữ liệu cục bộ này?")) { delete state.datasets[id]; if (state.datasetSearch) delete state.datasetSearch[id]; save(); render(); notify("Đã xóa dữ liệu cục bộ."); } return; }
    if (action === "download-script" && state.activeScript) { const item = state.activeScript; core().download(`tiktok-script-${Date.now()}.txt`, item.output || `${item.hook}\n\n${item.voiceover}\n\n${(item.shots || []).join("\n")}\n\n${item.caption}\n${item.cta}`); return; }
    if (action === "save-script-project" && state.activeScript) { try { const item = state.activeScript; setBusy("Đang lưu dự án…"); await publishing().saveProject({ title: item.topic, script: item.output || item.voiceover || "", captions: item.output ? "" : `${item.caption || ""}\n${item.cta || ""}`, aigc: item.provider && item.provider !== "local" ? true : Boolean(item.aigc), rights: { owned: true, musicConfirmed: false } }); await refresh(true); notify("Đã lưu dự án vào backend riêng của tài khoản."); } catch (error) { notify(error.message, "error"); } finally { busy = ""; render(); } return; }
  ...
  async function onSubmit(event) {
    ...
    if (kind === "script") {
      const input = Object.fromEntries(data); const fallback = core().buildScript(input);
      if (String(data.get("generator") || "ai") === "local") { state.activeScript = fallback; state.scripts = [...(state.scripts || []), fallback].slice(-20); save(); render(); return; }
      try {
        setBusy("AI đang viết hook, lời thoại và shot list…");
        const requestText = `Chủ đề: ${input.topic}\nThời lượng: ${input.duration} giây\nTone: ${input.tone}\nKhán giả: ${input.audience}\nViết kịch bản TikTok tiếng Việt nguyên bản có mốc thời gian, hook, voice-over, shot list, caption, CTA và hashtag. Không bịa trend hoặc số liệu.`;
        const action = await connections().creative(requestText, "tiktok-script", { creativity: 76, requireProvider: false });
        const script = { ...fallback, output: String(action.output || "").trim(), provider: action.provider || "local", model: action.model || "hh-local", source: action.provider === "local" ? "local-fallback" : "server-ai", aigc: action.provider !== "local" };
        state.activeScript = script; state.scripts = [...(state.scripts || []), script].slice(-20); save(); notify(action.provider === "local" ? "AI provider tạm chưa sẵn sàng; đã tạo bản offline." : "Đã tạo kịch bản bằng AI backend.");
      } catch (error) { state.activeScript = fallback; state.scripts = [...(state.scripts || []), fallback].slice(-20); save(); notify(`${error.message} Đã dùng bản offline.`, "error"); }
      finally { busy = ""; render(); }
      return;
    }
    if (kind === "subtitle-editor") { try { const cues = core().parseSubtitles(String(data.get("subtitleText") || "")); if (!cues.length) throw new Error("Không tìm thấy cue hợp lệ sau khi chỉnh sửa."); state.subtitle = { ...(state.subtitle || {}), name: state.subtitle?.name || "subtitle.srt", cues, editedAt: new Date().toISOString() }; save(); render(); notify(`Đã áp dụng ${cues.length} cue đã chỉnh.`); } catch (error) { notify(error.message, "error"); } return; }
    ...
    if (kind === "rename") {
      const prefix = String(data.get("prefix") || "tiktok").trim().replace(/[^\p{L}\p{N}_.-]+/gu, "-").replace(/^-+|-+$/g, "") || "tiktok"; const start = Math.max(0, Number(data.get("start") || 1));
      const rows = renameFiles.map((file, index) => { const dot = file.name.lastIndexOf("."); const extension = dot > 0 ? file.name.slice(dot) : ""; return { original_name: file.name, new_name: `${prefix}-${String(start + index).padStart(4, "0")}${extension.toLowerCase()}`, mime: file.type || "unknown", size_bytes: file.size, last_modified: new Date(file.lastModified || 0).toISOString() }; });
      core().download(`tiktok-rename-manifest-${Date.now()}.csv`, core().toCsv(rows), "text/csv;charset=utf-8"); notify(`Đã xuất manifest đổi tên cho ${rows.length} file; file gốc không bị sửa.`); return;
    }
    if (kind === "publish") {
      ...
      try { setBusy("Đang chuẩn bị tác vụ đã xác nhận…"); const result = await publishing().prepare(payload, remote, creatorInfoData, publishFile); const jobId = result.job?.id || ""; if (jobId) preparedUploads.set(jobId, { file: publishFile, media: { ...publishMedia }, name: publishFile.name, size: publishFile.size, type: publishFile.type }); await refresh(true); uploadRuntime = { jobId, status: result.job?.status || "ready", percent: 0, detail: payload.scheduledFor ? "Lịch nội bộ đã lưu · không tự upload" : "Đã chuẩn bị · bấm Bắt đầu upload để truyền đúng file", retry: 0 }; notify(result.note || "Đã chuẩn bị tác vụ. Video chưa được upload."); }
      catch (error) { notify(error.message, "error"); }
      finally { busy = ""; render(); }
    }
  }
  ...
  function onChange(event) {
    if (event.target.matches("[data-import]")) importFile(event.target);
    ...
    else if (event.target.matches("[data-rename-files]")) { renameFiles = Array.from(event.target.files || []).slice(0, 2000); render(); notify(`Đã chọn ${renameFiles.length} file để tạo rename manifest.`); }
      ...
      for (const name of ["ownBrand", "brandedContent"]) { const input = form?.elements?.namedItem(name); if (input) { input.disabled = !enabled; if (!enabled) input.checked = false; } }
      const policy = form?.elements?.namedItem("brandedPolicyConfirmed"); if (policy) { policy.disabled = true; policy.checked = false; }
    }
      ...
      const policy = form?.elements?.namedItem("brandedPolicyConfirmed"); if (policy) { policy.disabled = !branded; if (!branded) policy.checked = false; }
  ...
  function onInput(event) {
    if (!event.target.matches("[data-dataset-search]")) return;
    const id = event.target.dataset.datasetSearch; state.datasetSearch = { ...(state.datasetSearch || {}), [id]: event.target.value }; save();
    const card = event.target.closest(".ttg-workspace")?.querySelector(`[data-dataset-table="${core().cleanId(id)}"]`);
    if (card) { const rows = dataset(id); card.innerHTML = id === "trends" ? trendResults(filteredRows(rows, id)) : `<h3>${esc(activeWorkspace().title)} · dữ liệu lọc</h3>${genericTable(rows, 50, id)}`; }
  }
  ...
  function mount(target) {
    ...
    host.addEventListener("click", onClick); host.addEventListener("submit", onSubmit); host.addEventListener("change", onChange); host.addEventListener("input", onInput); global.addEventListener("online", render); global.addEventListener("offline", render);
    handleOauthResult(); render(); refresh(true);
  }
  function unmount() {
    if (!host) return; host.removeEventListener("click", onClick); host.removeEventListener("submit", onSubmit); host.removeEventListener("change", onChange); host.removeEventListener("input", onInput); global.removeEventListener("online", render); global.removeEventListener("offline", render); global.speechSynthesis?.cancel();
    if (activeMediaUrl) URL.revokeObjectURL(activeMediaUrl); if (publishFileUrl) URL.revokeObjectURL(publishFileUrl); uploadController?.abort(); uploadController = null; activeMediaFile = null; activeMediaUrl = ""; publishFile = null; publishFileUrl = ""; publishMedia = null; preparedUploads.clear(); creatorInfoData = null; renameFiles = []; clearTimeout(toastTimer); toastState = null; host.innerHTML = ""; host = null; state = null; profile = null; accountVideos = [];
  }
