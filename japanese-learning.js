(() => {
  ...
  function dictionarySearch(query = "") {
    ...
    return WORDS.filter((item) => [item.word, item.kana, item.romaji, item.meaning, item.example, item.exampleVi].some((value) => normalizeSearch(value).includes(key)) || (kana !== key && item.kana.includes(kana))).slice(0, 30);
  }
  ...
  const defaults = () => ({ version: VERSION, view: "dashboard", level: "N5", dailyGoal: 20, streak: 0, lastStudyDay: "", xp: 0, dailyActivity: {}, saved: {}, reviews: {}, customWords: [], history: [], completedReadings: {}, testHistory: [], searchQuery: "", selectedKanji: "日", selectedGrammarLevel: "all", selectedReading: "r-n5", furigana: true, theme: "light" });
  function readState() {
    try {
  ...
  function jlptQuestions(level) {
    const pool = WORDS.filter(item=>item.level===level); const grammar = GRAMMAR.filter(item=>item.level===level); const questions=[];
    pool.slice(0,5).forEach((item,index)=>{ const distract=WORDS.filter(row=>row.id!==item.id).slice(index,index+3).map(row=>row.meaning); questions.push({prompt:`「${item.word}」の意味は何ですか。`,answer:item.meaning,options:[item.meaning,...distract].sort(()=>.5-Math.random()),explanation:`${item.word}（${item.kana}）: ${item.meaning}`}); });
  ...
  global.HHJapanese=Object.freeze({mount,unmount,views:VIEWS.map(([id,label])=>({id,label})),dictionarySearch,romajiToHiragana,hiraganaToKatakana,conjugateVerb,scoreDictation,dailyPlan,words:WORDS,kanji:KANJI,grammar:GRAMMAR,readings:READINGS});
})();
