...
import { WORLD_PROFILES, profileFor, surfaceMaterial, atmosphereMaterial, cloudMaterial, ringMaterial, orbitPosition } from './galaxy-celestial-materials.mjs?v=3';
...
export function buildLandmark(profile, track) {
 ...
 switch(profile.motif){
  ...
  case 'archipelago': {
   const rocks=new T.InstancedMesh(track(new T.IcosahedronGeometry(.32,0)),metal,14),matrix=new T.Matrix4();
   for(let i=0;i<14;i++){matrix.compose(new T.Vector3().fromArray(orbitPosition(2.8+(i%3)*.3,i*2.39,.3)),new T.Quaternion().setFromEuler(new T.Euler(i,i*.7,0)),new T.Vector3(1+i%3*.3,.4,1));rocks.setMatrixAt(i,matrix);}group.add(rocks);break;
  }
  case 'lattice': {
  ...
  case 'station': {
   const modules=new T.InstancedMesh(track(new T.BoxGeometry(.3,.18,.6)),metal,8),matrix=new T.Matrix4();
   for(let i=0;i<8;i++){const a=i*Math.PI/4;matrix.compose(new T.Vector3().fromArray(orbitPosition(2.9,a,.1)),new T.Quaternion().setFromAxisAngle(new T.Vector3(0,1,0),-a),new T.Vector3(1,1,1));modules.setMatrixAt(i,matrix);}group.add(modules);add(torus(2.9,.06)).rotation.x=Math.PI/2;break;
  }
 }
 return group;
}
...
export function mount(host, options = {}) {
 ...
 try {
  ...
  scene=new T.Scene();const camera=new T.PerspectiveCamera(38,1,.1,220);camera.position.set(0,.6,11);camera.lookAt(0,0,0);
  ...
  const sky=createDeepSpace(track,{color:profile.color,accent:profile.accent,seed:material.uniforms.seed.value*991+17});scene.add(sky.group);
  function pose(){
   ...
   sky.update(elapsed);material.uniforms.time.value=elapsed;
  }
  function renderOnce(){
  ...
  function resize(){
   ...
   sky.setPixelRatio(Math.min(win.devicePixelRatio||1,quality==='high'?1.5:quality==='low'?1:1.25));
  ...
  function applyQuality(){sky.setQuality({low:'economy',mid:'balanced',high:'cinematic'}[quality]);shell.visible=quality!=='low';if(clouds)clouds.visible=quality!=='low';landmark.visible=quality!=='low';resize();}
  function tick(now){
