(() => {
  ...
  function imageWorkspace(name) {
    ...
    const kind = compressor ? "compressor" : converter ? "converter" : "toolkit";
    ...
    const pipeline = compressor ? [["01", "Phân tích"], ["02", "Tối ưu pixel"], ["03", "So sánh dung lượng"]] : converter ? [["01", "Đọc nguồn"], ["02", "Chuyển codec"], ["03", "Kiểm tra đầu ra"]] : [["01", "Biến đổi"], ["02", "Chỉnh màu"], ["03", "Xuất canvas"]];
    ...
    return `${commonHeader(name, description)}<div class="md-image-pipeline" aria-label="Quy trình ${esc(name)}">${pipeline.map(([step, label]) => `<span><b>${step}</b>${label}</span>`).join("")}</div><div class="md-image-layout md-image-layout--${kind}"><section>${uploadBox("image/*", false, compressor ? "Thả ảnh cần tối ưu" : converter ? "Chọn ảnh cần chuyển đổi" : "Mở ảnh vào canvas")}<div class="md-control-grid">${controls}</div>${actionBar("image-process", compressor ? "Nén và đo kết quả" : converter ? "Chuyển đổi định dạng" : "Áp dụng lên canvas")}</section><section class="md-preview-panel"><header><strong>${compressor ? "So sánh tối ưu" : converter ? "Đầu ra định dạng mới" : "Canvas chỉnh sửa"}</strong><span data-md-image-meta>Chưa có ảnh</span></header><div class="md-canvas-stage"><canvas data-md-canvas></canvas><div data-md-placeholder>${compressor ? "Kết quả nén sẽ hiển thị tại đây" : converter ? "Ảnh sau chuyển đổi sẽ xuất hiện tại đây" : "Ảnh sẽ hiển thị tại đây"}</div></div><div class="md-result-stats" data-md-result-stats></div></section></div>`;
  }
