(function initHHMediaAudioStudio(root, factory) {
  ...
  const LIMITS = Object.freeze({ tracks: 12, markers: 100, fileBytes: 256 * 1024 * 1024, renderBytes: 192 * 1024 * 1024 });
  let active = null;
  ...
  function updateTrackList(input, operation = {}) {
    ...
    if (operation.type === "add" && tracks.length < LIMITS.tracks) {
      ...
      const source = tracks[index], copy = normalizeTrack({ ...source, id: cleanText(operation.newId || `${source.id}-copy`, 80), name: `${source.name} · bản sao` }, tracks.length);
  ...
  function renderEstimate(tracks, settings) {
    const sources = (Array.isArray(tracks) ? tracks : []).filter((track) => track?.buffer?.length);
    const duration = sources.reduce((maximum, track) => Math.max(maximum, Number(track.buffer.duration) || 0), 0);
  ...
  async function renderMixAsync(context, tracks, settings, options = {}) {
    ...
    const filterState = Array.from({ length: estimate.channels }, () => ({ input: 0, output: 0 }));
    let peak = 0;
      ...
      for (let channel = 0; channel < estimate.channels; channel += 1) {
        ...
        for (let frame = from; frame < to; frame += 1) {
          ...
          for (const track of activeTracks) {
            const source = track.buffer, sourceFrame = Math.floor(absoluteTime * source.sampleRate);
            if (sourceFrame >= source.length) continue;
            ...
            sample += (source.getChannelData(sourceChannel)[sourceFrame] || 0) * Math.pow(10, track.gain / 20) * panGain * headroom;
          }
  ...
  function mount(host, options) {
    ...
    host.innerHTML = `<section class="mas-studio" data-media-audio-studio>
      ...
      <div class="mas-grid"><main>
        ...
        <section class="mas-podcast"><header><small>PODCAST DELIVERY</small><strong>Metadata & chapters</strong></header><div class="mas-podcast-grid"><label>Tiêu đề<input value="${escapeHtml(session.state.podcast.title)}" maxlength="160" data-mas-podcast="title"></label><label>Chương trình<input value="${escapeHtml(session.state.podcast.show)}" maxlength="160" data-mas-podcast="show"></label><label>Tác giả<input value="${escapeHtml(session.state.podcast.author)}" maxlength="120" data-mas-podcast="author"></label><label>Số tập<input type="number" min="1" max="99999" value="${session.state.podcast.episode}" data-mas-podcast="episode"></label></div><label>Mô tả<textarea maxlength="600" rows="3" data-mas-podcast="description">${escapeHtml(session.state.podcast.description)}</textarea></label><div class="mas-podcast-actions"><button type="button" data-mas-chapter disabled>＋ Chapter tại In</button><button type="button" data-mas-chapters disabled>Chapters JSON</button><button type="button" data-mas-rss disabled>RSS item</button></div><div class="mas-chapter-list" data-mas-chapter-list></div></section>
    ...
    const duration = () => session.tracks.reduce((maximum, track) => Math.max(maximum, Number(track.buffer?.duration) || 0), 0);
    ...
    const updateLabels = () => {
      ...
      const enabled = Boolean(session.tracks.length); host.querySelectorAll("[data-mas-play],[data-mas-stop],[data-mas-export],[data-mas-save-bin],[data-mas-silence],[data-mas-chapter],[data-mas-chapters],[data-mas-rss]").forEach((button) => { button.disabled = !enabled || (Boolean(session.busy) && !button.matches("[data-mas-stop]")); });
    ...
    const loadFiles = async (input) => {
      ...
      try {
        for (const file of valid) {
          ...
          session.tracks = updateTrackList(session.tracks, { type: "add", track: { id, name: file.name, file, buffer, analysis: estimateLoudness(buffer) } }); session.selectedTrackId = id; loaded += 1; setStatus(`Đang giải mã ${loaded}/${valid.length}…`);
        }
        ...
        renderTracks(); updateLabels(); renderChapters(); setStatus(`${session.tracks.length} rãnh sẵn sàng`); announce(`Đã thêm ${loaded} rãnh và dựng waveform từ audio thật.`, "success");
    ...
    const runRender = async (mode) => {
      ...
      try {
        ...
        if (mode === "play") {
          const source = context.createBufferSource(); source.buffer = processed; source.connect(context.destination); session.playing = source; session.busy = ""; session.renderController = null; studio?.classList.remove("is-rendering"); studio?.classList.add("is-playing"); source.onended = () => { if (session.playing === source) { session.playing = null; studio?.classList.remove("is-playing"); setStatus(`${session.tracks.length} rãnh sẵn sàng`); updateLabels(); } }; source.start(); setStatus("Đang phát bản mix đã xử lý"); updateLabels();
        } else {
    ...
    const saveToBin = async () => {
      ...
      try { store = session.options.mediaApi.createStore(); await store.ready(); const project = (await store.listProjects())[0] || await store.saveProject({ name: "Universal Media Project" }); for (const track of session.tracks) await store.saveAsset({ projectId: project.id, name: track.file.name, type: track.file.type, size: track.file.size, lastModified: track.file.lastModified, blob: track.file }); announce(`Đã thêm ${session.tracks.length} audio nguồn vào Global Media Bin.`, "success"); }
    ...
    host.addEventListener("click", async (event) => {
      ...
      const trackAction = event.target.closest("[data-mas-track-action]"); if (trackAction) {
        ...
        if (action === "remove") session.tracks = updateTrackList(session.tracks, { type: "remove", id });
        if (action === "duplicate") session.tracks = updateTrackList(session.tracks, { type: "duplicate", id, newId: `${id}-${Date.now()}` });
        if (action === "up" || action === "down") session.tracks = updateTrackList(session.tracks, { type: "move", id, delta: action === "up" ? -1 : 1 });
        if (action === "mute" || action === "solo") session.tracks = updateTrackList(session.tracks, { type: "update", id, patch: { [action === "mute" ? "muted" : "solo"]: !track[action === "mute" ? "muted" : "solo"] } });
        if (!session.tracks.some((item) => item.id === session.selectedTrackId)) session.selectedTrackId = session.tracks[0]?.id || "";
        session.state.end = duration(); renderTracks(session.selectedTrackId); updateLabels(); return;
      }
    ...
    host.addEventListener("change", (event) => { if (event.target.matches("[data-mas-file]")) { loadFiles(event.target.files); event.target.value = ""; } }, { signal: controller.signal });
    host.addEventListener("input", (event) => {
      const trackSetting = event.target.dataset.masTrackSetting;
      if (trackSetting) { session.tracks = updateTrackList(session.tracks, { type: "update", id: event.target.dataset.trackId, patch: { [trackSetting]: Number(event.target.value) } }); const output = event.target.closest("label")?.querySelector("output"); if (output) output.textContent = `${event.target.value}${trackSetting === "gain" ? " dB" : "%"}`; return; }
      ...
      const podcast = event.target.dataset.masPodcast; if (podcast) { session.state.podcast[podcast] = podcast === "episode" ? clamp(event.target.value, 1, 99999) : cleanText(event.target.value, podcast === "description" ? 600 : 160); save(session.state); return; }
    ...
    renderTracks(); renderChapters(); updateLabels();
    return Object.freeze({ getState: () => ({ ...normalizeState(session.state), tracks: session.tracks.map((track) => ({ id: track.id, name: track.name, gain: track.gain, pan: track.pan, muted: track.muted, solo: track.solo })) }), loadFile: (file) => loadFiles([file]), loadFiles, stop: stopPlayback, unmount });
  }
  function unmount() {
  ...
  return Object.freeze({ STORAGE_KEY, LIMITS, normalizeState, normalizeTrack, updateTrackList, trimRange, encodeWav, encodeWavAsync, processBuffer, renderEstimate, renderMixAsync, estimateLoudness, findSpeechBounds, findSpeechBoundsAsync, chaptersJson, rssItem, mount, unmount });
});
