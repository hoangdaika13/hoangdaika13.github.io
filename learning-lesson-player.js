(function learningLessonPlayerFactory(globalScope) {
  ...
  function buildLessonContent(lesson) {
    ...
    return [
      {
        id: "article", type: "article", label: "Khởi động", skillId: "reading", title: `${title}: ngữ cảnh`,
      ...
      {
        id: "scenario", type: "scenario", label: "Tình huống", skillId: "speaking", title: "Chọn phản hồi phù hợp",
  ...
  function evaluateStep(step, answer) {
    ...
    if (!step || ["article", "video", "record"].includes(step.type)) {
  ...
  function renderStepper(runtime) {
    return runtime.steps.map((step, index) => `<button type="button" class="hlp-step-dot${index === runtime.stepIndex ? " is-current" : ""}${runtime.results[step.id] ? " is-done" : ""}" data-hlp-go="${index}" aria-label="Bước ${index + 1}: ${escapeHtml(step.label)}"${index === runtime.stepIndex ? ' aria-current="step"' : ""}><span>${index + 1}</span><small>${escapeHtml(step.label)}</small></button>`).join("");
  }
  ...
  function renderStep(runtime, step) {
    ...
    if (step.type === "article") return `<article class="hlp-reading"><p>${escapeHtml(step.body)}</p><blockquote lang="en">${escapeHtml(step.example)}</blockquote><button type="button" class="hlp-primary" data-hlp-action="mark-step">Đã đọc xong</button></article>`;
    ...
    if (step.type === "flashcard") return `<div class="hlp-flashcard"><button type="button" data-hlp-action="flip-card" aria-expanded="${runtime.cardOpen ? "true" : "false"}"><small>Mặt trước</small><strong lang="en">${escapeHtml(step.prompt)}</strong><span>${runtime.cardOpen ? escapeHtml(step.answer) : "Chạm hoặc nhấn Enter để lật"}</span></button><p lang="en">${escapeHtml(step.example)}</p><div class="hlp-rating" role="group" aria-label="Tự đánh giá thẻ nhớ">${[["again", "Quên"], ["hard", "Khó"], ["good", "Tốt"], ["easy", "Dễ"]].map(([value, label]) => `<button type="button" data-hlp-flash-rating="${value}">${label}</button>`).join("")}</div></div>`;
    ...
    if (step.type === "drag") {
      ...
      const remaining = step.tokens.filter((token, index) => !selected.some((chosen, chosenIndex) => chosen === token && chosenIndex === index));
    ...
    if (step.type === "listen") return `<div class="hlp-listen"><button type="button" class="hlp-speak" data-hlp-action="speak" aria-label="Phát câu tiếng Anh"><span aria-hidden="true">◖</span> Nghe câu</button>${step.options.map((option) => `<label><input type="radio" name="listen-answer" value="${escapeHtml(option)}" data-hlp-choice="${step.id}"${answer === option ? " checked" : ""}><span lang="en">${escapeHtml(option)}</span></label>`).join("")}<button type="button" class="hlp-primary" data-hlp-action="check-choice">Kiểm tra</button></div>${resultMarkup(result)}`;
    if (step.type === "record") return `<div class="hlp-record"><p class="hlp-question" lang="en">${escapeHtml(step.prompt)}</p><div class="hlp-record-actions"><button type="button" class="hlp-speak" data-hlp-action="speak-record">Nghe mẫu</button><button type="button" class="hlp-primary" data-hlp-action="${runtime.recording ? "stop-recording" : "start-recording"}">${runtime.recording ? "Dừng ghi âm" : "Bắt đầu ghi âm"}</button></div><p class="hlp-permission-note">Micro chỉ được yêu cầu sau khi bạn bấm nút. Bản ghi không tự tải lên.</p><audio controls hidden data-hlp-record-playback aria-label="Bản ghi phát âm"></audio><div class="hlp-live" aria-live="polite">${escapeHtml(runtime.recordStatus || "Chưa ghi âm")}</div>${runtime.recorded ? `<button type="button" data-hlp-action="mark-record">Đã nghe và tự đánh giá</button>` : ""}</div>${resultMarkup(result)}`;
  ...
  function render(runtime, focusSelector) {
    ...
    runtime.host.innerHTML = `<section class="hlp-shell" data-hlp-root>
  ...
  function recordMistake(runtime, step, result) {
    ...
    runtime.store.update((state) => {
      ...
      state.mistakes.unshift({ id: runtime.core.uid?.("mistake") || `mistake-${Date.now()}`, lessonId: runtime.lesson.id, skillId: step.skillId, prompt: step.prompt || step.title, answer: result.expected, userAnswer: result.userAnswer, createdAt: now, resolved: false });
      state.reviews = Array.isArray(state.reviews) ? state.reviews : [];
      const card = { id: runtime.core.uid?.("review") || `review-${Date.now()}`, prompt: step.prompt || step.title, answer: result.expected, trackId: runtime.lesson.trackId, skillId: step.skillId, difficulty: runtime.lesson.difficulty || 3, stability: 1, intervalDays: 0, lapses: 0, dueAt: now };
      state.reviews.unshift(typeof runtime.core.scheduleReview === "function" ? runtime.core.scheduleReview(card, "again", Date.now(), state.profile?.retentionGoal) : card);
      return state;
    });
  }

  function setResult(runtime, step, answer, override) {
    runtime.answers[step.id] = answer;
    const result = override || evaluateStep(step, answer);
    runtime.results[step.id] = result;
    recordMistake(runtime, step, result);
  ...
  async function startRecording(runtime) {
    ...
    try {
      ...
      if (typeof Recognition === "function") {
        try { runtime.recognition = new Recognition(); runtime.recognition.lang = "en-US"; runtime.recognition.start(); } catch { runtime.recognition = null; }
      }
      render(runtime);
      return true;
    } catch (error) {
  ...
  function finishRecording(runtime) {
    ...
    if (runtime.recordChunks?.length && runtime.scope?.Blob && runtime.scope?.URL?.createObjectURL) {
      ...
      runtime.recordStatus = "Đã ghi xong. Hãy nghe lại trước khi tự đánh giá.";
  ...
  function handleClick(runtime, event) {
    ...
    if (target.dataset.hlpFlashRating) {
      const correct = target.dataset.hlpFlashRating !== "again";
      return setResult(runtime, step, target.dataset.hlpFlashRating, { correct, score: { again: 20, hard: 60, good: 85, easy: 100 }[target.dataset.hlpFlashRating], userAnswer: target.dataset.hlpFlashRating, expected: step.answer, explanation: step.explanation });
    }
    ...
    else if (action === "mark-record") setResult(runtime, step, "recorded");
  ...
  function loadLesson(runtime, lessonId) {
    ...
    runtime.cardOpen = false; runtime.completed = false; runtime.next = null; runtime.status = draft ? "Đã khôi phục bản học tự động lưu." : "";
    render(runtime, "[data-hlp-stage]");
  }
  ...
  function cleanupMedia(runtime) {
    ...
    runtime.localVideoUrl = ""; runtime.recordingUrl = "";
  }
  ...
  function mount(host, options = {}) {
    ...
    const runtime = {
      ...
      recording: false, recorded: false, recordStatus: "", recordChunks: [], status: draft ? "Đã khôi phục bản học tự động lưu." : ""
    };
  ...
  if (typeof module !== "undefined" && module.exports) module.exports = Object.freeze({
    ...
    evaluateStep, nextLesson, loadDraft, saveDraft, clearDraft, startRecording, cleanupMedia, publicApi
  });
