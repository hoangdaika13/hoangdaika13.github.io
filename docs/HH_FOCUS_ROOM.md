...
- Bicolor cat and Shiba companions use two self-hosted, CC BY 4.0 GLB models with denser anatomy, PBR fur textures and real skeletal animation. Automatic behavior alternates a bounded walk, long rest and idle; users can force Rest or Walk. Transitions crossfade instead of squashing the mesh, while the walk overlay moves paired legs, spine, head and tail with eased turns. Balanced mode caps at 24 FPS/DPR 1, High at 30 FPS/DPR 1.5, and eco/data saver/reduced motion/hidden tabs/WebGL failure use the lightweight fallback with no active render loop.
...
The pet collection keeps six opt-in photographic scenes and adds selectable 3D Bicolor cat/Shiba companions to every environment. The Shiba uses its supplied standing and lying clips; the cat keeps its supplied full-body idle/expression clip. Walking adds a restrained quadruped gait over the live source clip, with eased direction changes and no whole-body stretch. Automatic behavior spends most of its 44-second cycle resting, with a short walk and idle transition; explicit Rest and Walk modes are also persisted per account. Eco, data saver, paused motion, hidden tabs and system reduced motion remove WebGL work completely. The previous depth portrait remains available only as the fallback for legacy photographic pet scenes when no 3D companion is selected.
...
- Select Bicolor Cat and Shiba in Balanced and High quality; verify Auto, Rest and Walk, then hide/show the tab, pause motion, enable data saver and leave the route to confirm the GLB renderer stops and disposes.
