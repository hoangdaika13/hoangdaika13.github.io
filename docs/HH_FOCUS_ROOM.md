...
All new scene images are original project assets generated for HH Platform. Their hashes, prompt boundary and generation provenance are recorded in `assets/focus-room/README.md`. Ambient audio is synthesized locally; no third-party audio file is shipped.
...
The pet collection adds six opt-in scenes: rainy attic cat, retriever sunroom, fireside library cat, spring veranda dog, Vietnamese riverside cat and lakeside cabin puppy. These scenes use a feathered local duplicate of the same project image for an extremely small breathing-scale illusion plus existing bounded camera parallax; no synthetic animal skeleton or distracting locomotion is fabricated. Eco, paused-motion, hidden-tab and system reduced-motion states remove the local animation completely.
