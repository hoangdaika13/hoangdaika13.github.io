...
- A 230-scene library: the original 30 project-local scenes plus exactly 200 Atlas presets built from 20 owned backgrounds and ten lighting/atmosphere states. Atlas 200 is presented truthfully as presets, not 200 independent photographs.
...
- Bicolor cat and Shiba companions use two self-hosted, CC BY 4.0 GLB models with denser anatomy, PBR fur textures and real skeletal animation. Four cat coats and four Shiba coats are generated locally from the licensed diffuse maps while preserving their normal/roughness detail; the UI names them as coat variants, not separate breeds. Automatic behavior alternates a bounded walk, long rest and idle; users can force Rest or Walk. Clicking or keyboard-activating the projected pet hit target records a real account-scoped interaction: the cat performs a restrained skeletal nuzzle/head tilt/tail wave, while Shiba alternates its supplied `shake` and `rollover` clips before crossfading back. Balanced mode caps at 24 FPS/DPR 1, High at 30 FPS/DPR 1.5, and eco/data saver/reduced motion/hidden tabs/WebGL failure use the lightweight fallback with no active render loop.
...
7. [Bicolor Cat by kenchoo](https://sketchfab.com/3d-models/bicolor-cat-e623a618ca344a8393d7ba4d63ec23cf) and the [vr-cats distribution](https://github.com/code4fukui/vr-cats) — CC BY 4.0 rigged domestic cat with diffuse, normal and roughness textures. The creator attribution appears in the scene UI, asset manifest and third-party notice.
8. [Animated Dog, Shiba Inu by quander](https://sketchfab.com/3d-models/animated-dog-shiba-inu-9abfce885a834399b2c3ccaed51cd474) and the [bitcoinweighin distribution](https://github.com/hmgovt/bitcoinweighin) — CC BY 4.0 rigged Shiba with five embedded body/expression clips and PBR textures. Attribution is retained in all three credit surfaces.
...
All new scene images are original project assets generated for HH Platform. Their hashes, prompt boundary and generation provenance are recorded in `assets/focus-room/README.md`. Ambient and lo-fi beds are synthesized locally. Shipped third-party media consists of two Kimiko Ishizaka CC0 piano performances plus the kenchoo Bicolor Cat and quander Shiba Inu under CC BY 4.0; source URLs, licenses, attribution and hashes are documented in the asset manifest and `assets/focus-room/pets/THIRD_PARTY_NOTICES.md`.

## Atlas 200 and pet expansion

The built-in library now contains the original rooftop sunrise, rainy greenhouse, alpine lake dawn, old university reading hall, Nordic cabin morning, Vietnamese rice-terrace veranda, original autumn garden room and moonlit observatory additions. Four further Pet Sanctuary 3D scenes provide a rainy reading conservatory, moss garden pavilion, moonlit fireside library and coastal sunset studio. They use original generated backgrounds without embedded animals, wide floor lanes and scene-matched cat/dog defaults so the live model remains grounded instead of duplicating an animal already in the image. Rain, forest, moonlight and sunset light profiles adjust the companion's hemisphere, key/rim balance, exposure and contact shadow to match each environment. Atlas 200 then combines 20 local backgrounds—including the rainy garden sanctuary—with dawn, mist, clear day, golden hour, sunset, blue hour, quiet night, soft rain, starlight and pet-companion states. Every combination has its own stable ID, title, grade, crop, effect, sound suggestion and optional pet assignment.

The pet collection keeps six opt-in generated portrait scenes, adds four empty pet-ready sanctuaries and offers eight selectable coat presentations—orange/white, silver, cream and midnight cat; golden, cream, red and sesame Shiba—to every environment. These choices deliberately reuse the two credited source rigs instead of claiming eight independently sourced breeds. The Shiba uses supplied standing, lying, shake and rollover clips; the cat keeps its supplied full-body idle/expression clip plus small reversible joint offsets for direct interaction. Walking adds a restrained quadruped gait over the live source clip: the cat uses a four-beat footfall sequence, while both rigs add paw flex, neck/head stabilization, spinal weight transfer, body lift and species-specific tail follow-through. Its side-floor path is a continuous perspective ellipse: the model walks laterally, advances toward the camera, retreats into the scene and turns along the path tangent without stopping or snapping at screen edges. The depth range is large enough to make the approach and retreat visible, while the narrower lateral range and edge-biased center keep the companion in the open lane beside the clock. Perspective changes size naturally, the ground-contact height follows depth and a damped return walk brings the companion back to its resting point before the resting clip takes over. A very small vertex-normal coat ripple is applied only to compatible PBR materials; scene profiles vary its amplitude for rain, forest, moonlight and coastal breeze without simulating individual fur strands or altering the low-power fallback. Balanced quality targets 30 FPS and High targets 60 FPS. Automatic behavior spends most of its 44-second cycle resting, with a short walk and idle transition; explicit Rest and Walk modes, interaction count and latest interaction time persist per account. The transparent semantic pet button follows the projected model while remaining below the clock, header, dock and panels, and the full WebGL canvas still cannot capture pointer input. Below 620px the walk path and depth range are constrained further beside the narrower clock card, keeping both the animal and its direct hit target reachable. Eco, data saver, paused motion, hidden tabs and system reduced motion remove WebGL work and pet interaction completely. The previous depth portrait remains available only as the fallback for the six legacy generated pet portraits when no live 3D companion is selected.
...
- Verify the exported catalog contains 230 unique IDs, four `Pet Sanctuary 3D` scenes and exactly 200 `Atlas 200` presets, then page through all results in 24-card increments.
- Select all four Bicolor-cat coat variants and all four Shiba coat variants in Balanced and High quality; verify Auto, Rest and Walk. Click and keyboard-activate the pet, confirm cat nuzzle/tail response and Shiba shake/rollover crossfades, and confirm the account interaction count survives reload. Then hide/show the tab, pause motion, enable data saver and leave the route to confirm the hit target hides and the GLB renderer stops and disposes.
