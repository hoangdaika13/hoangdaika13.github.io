...
The pet collection keeps six opt-in photographic scenes and adds eight selectable coat presentations—orange/white, silver, cream and midnight cat; golden, cream, red and sesame Shiba—to every environment. These choices deliberately reuse the two credited source rigs instead of claiming eight independently sourced breeds. The Shiba uses supplied standing, lying, shake and rollover clips; the cat keeps its supplied full-body idle/expression clip plus small reversible joint offsets for direct interaction. Walking adds a restrained quadruped gait over the live source clip, with eased direction changes and no whole-body stretch. Automatic behavior spends most of its 44-second cycle resting, with a short walk and idle transition; explicit Rest and Walk modes, interaction count and latest interaction time persist per account. The transparent semantic pet button follows the projected model while remaining below the clock, header, dock and panels, and the full WebGL canvas still cannot capture pointer input. Below 620px the walk path is constrained to the free lane beside the narrower clock card, keeping both the animal and its direct hit target reachable. Eco, data saver, paused motion, hidden tabs and system reduced motion remove WebGL work and pet interaction completely. The previous depth portrait remains available only as the fallback for legacy photographic pet scenes when no 3D companion is selected.
