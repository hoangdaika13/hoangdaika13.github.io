...
- A 224-scene library: 24 active project-local scenes plus exactly 200 Atlas presets built from 20 owned backgrounds and ten lighting/atmosphere states. Atlas 200 is presented truthfully as presets, not 200 independent photographs.
...
- Fourteen procedural Web Audio channels rebuilt as longer, differently timed sound beds with scene-specific filtering, gentle gain drift, restrained stereo placement and sparse natural transients.
...
- Animal scenes, 3D companion controls, interaction tracking and animal-only sound channels were removed in release v1007. Existing timer, notes, tasks, layouts, mixes and account-scoped study history remain compatible; obsolete saved companion fields are ignored.
...
4. [mrdoob/three.js](https://github.com/mrdoob/three.js) — MIT-licensed WebGL library already bundled by the repository. An earlier Focus Room release used it for an optional depth layer; release v1007 removed that runtime.
...
7. [Bicolor Cat by kenchoo](https://sketchfab.com/3d-models/bicolor-cat-e623a618ca344a8393d7ba4d63ec23cf) and the [vr-cats distribution](https://github.com/code4fukui/vr-cats) — historical CC BY 4.0 asset retained offline for provenance but no longer loaded by Focus Room.
8. [Animated Dog, Shiba Inu by quander](https://sketchfab.com/3d-models/animated-dog-shiba-inu-9abfce885a834399b2c3ccaed51cd474) and the [bitcoinweighin distribution](https://github.com/hmgovt/bitcoinweighin) — historical CC BY 4.0 asset retained offline for provenance but no longer loaded by Focus Room.
...
All active scene images are original project assets generated for HH Platform. Their hashes, prompt boundary and generation provenance are recorded in `assets/focus-room/README.md`. Ambient and lo-fi beds are synthesized locally. Active third-party media is limited to two Kimiko Ishizaka CC0 piano performances. Historical animal asset attribution remains in `assets/focus-room/pets/THIRD_PARTY_NOTICES.md` even though those files are no longer requested.

## Atlas 200 and calm sanctuary environments

The built-in library contains 24 active local environments plus 200 Atlas presets. Four animal-free renders have been retained as Nhà kính đọc sách, Hiên vườn rêu, Thư viện trăng and Studio biển hoàng hôn under the `Calm Sanctuary` collection. Atlas 200 combines 20 local backgrounds with ten stable lighting and atmosphere treatments; the legacy `pet-companion` mood identifier remains only to preserve saved scene IDs and now renders a generic soft-light state with no animal metadata or runtime.

Six backgrounds that visibly contained dogs or cats are no longer part of the exported scene catalog. The 3D renderer, animal interaction controls and animal-only sound channels are not mounted or exported. Historical media files and attribution notices remain in the repository for provenance, but the Focus Room does not request them.
...
- Verify the exported catalog contains 224 unique IDs, four `Calm Sanctuary` scenes and exactly 200 `Atlas 200` presets, then page through all results in 24-card increments.
- Verify no scene metadata, filter, setting, canvas, interaction control or ambient channel exposes a dog or cat, including after loading an older account state.
