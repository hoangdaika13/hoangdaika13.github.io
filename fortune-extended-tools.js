(function fortuneExtendedToolsModule(scope, factory) {
  ...
  function createPhysiognomyReflection(values = {}) {
    ...
    return { ok: true, observations, method: "Người dùng tự mô tả → tách quan sát/diễn giải → câu hỏi kiểm chứng", privacy: "Không dùng camera, không tải ảnh và không tạo mẫu sinh trắc học. Gemini chỉ nhận nhãn văn hóa đã tự chọn và câu hỏi kiểm chứng, không nhận ảnh hoặc dữ liệu định danh.", limitations: ["Không suy luận nhân cách, trí tuệ, đạo đức, sức khỏe, dân tộc hoặc vận mệnh từ khuôn mặt.", "Các liên tưởng tướng pháp được trình bày như lịch sử văn hóa, không phải kết luận khoa học."] };
  }
  function analyzeDream(textValue, emotion = "curious") {
    ...
    return { ok: true, emotion: String(emotion || "curious"), wordCount: text.split(/\s+/).filter(Boolean).length, matches, prompts: ["Chi tiết nào rõ nhất ngay sau khi tỉnh dậy?", "Cảm xúc trong mơ giống tình huống thật nào gần đây?", "Có cách giải thích đời thường nào trước khi gán ý nghĩa biểu tượng?", "Bạn muốn thử một hành động nhỏ nào sau khi ghi lại?"], privacy: "Nội dung nguyên văn chỉ được xử lý trong trình duyệt và không tự lưu. Gemini chỉ nhận cảm xúc đã chọn cùng các mô-típ đã khớp cục bộ.", limitations: ["Giải mã giấc mơ không dự báo tương lai và không chẩn đoán sức khỏe tâm thần.", "Nếu ác mộng kéo dài hoặc gây khổ sở, nên trao đổi với chuyên gia phù hợp."] };
  }
