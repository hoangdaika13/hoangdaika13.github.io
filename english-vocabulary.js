(() => {
  ...
  const renderLesson = (instance, state) => {
    ...
    else if (stepId === "recognize") task = `<article class="hhev-word-question"><small>CHỌN NGHĨA ĐÚNG</small><h3>${esc(word.term)}</h3><div>${lessonChoices(word, lesson).map((choice) => `<button type="button" data-hhev-choice="${esc(choice)}" data-answer="${esc(word.meaning)}">${esc(choice)}</button>`).join("")}</div><output data-hhev-feedback></output></article>`;
    ...
    else if (stepId === "typing") task = `<form class="hhev-word-type" data-hhev-type-answer="${esc(word.term)}"><small>TỰ GÕ TỪ</small><h3>${esc(word.meaning)}</h3><input name="answer" autocomplete="off" spellcheck="false" placeholder="Nhập từ tiếng Anh..." required><button class="primary" type="submit">Kiểm tra →</button><output data-hhev-feedback></output></form>`;
  ...
  const handleSubmit = (instance, event) => {
    const form = event.target.closest("[data-hhev-type-answer]"); if (!form) return;
    event.preventDefault(); const input = form.elements.answer; const correct = normalizeTerm(input.value) === normalizeTerm(form.dataset.hhevTypeAnswer); const state = instance.runtime.readState();
