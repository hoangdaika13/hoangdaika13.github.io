...
module.exports = async function facebookPageManager(req, res) {
  return withApi(req, res, async ({ db, body }) => {
    ...
    if (route === "status" && req.method === "GET") {
      const [allPages, setupRows, recentJobs, audits, campaignRows, groupRows, ruleRows, automationEvents] = await Promise.all([
      ...
      return res.status(200).json({
        ...
        capabilities: { automaticPageCreation: false, batchSetup: true, pageManagement: true, publish: true, schedule: true, comments: true, insights: true, campaigns: true, approvalWorkflow: true, pageGroups: true, perPageOverrides: true, webhook: true, automationRules: true },
    ...
    if (route === "campaigns/stage" && req.method === "POST") {
      ...
      await campaigns.updateOne({ _id: id, userId: user._id }, { $set: update });
      await writeAudit(db, user._id, `campaign:${requested}`, { status: requested, detail: campaign.name });
      return res.status(200).json({ campaign: publicCampaign({ ...campaign, ...update }) });
    }
...
module.exports.__test = Object.freeze({ PERMISSIONS, GRAPH_VERSION, MAX_BATCH_PUBLISH, MAX_SETUP_IMPORT, safeReturnHash, setupDoc, publicSetup, campaignDoc, publicCampaign, publicGroup, publicRule });
