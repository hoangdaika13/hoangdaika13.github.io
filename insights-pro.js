(() => {
  ...
  function decisionPanel(data) {
    const visits = data.funnel[0]?.value || 0;
    const actions = data.funnel[1]?.value || 0;
    const submits = data.funnel[3]?.value || 0;
    const conversions = data.funnel[4]?.value || 0;
    const engagementRate = visits ? Math.round(actions / visits * 100) : 0;
    const submitRate = visits ? Math.round(submits / visits * 100) : 0;
    const conversionRate = visits ? Math.round(conversions / visits * 100) : 0;
    const errorCount = data.errors.reduce((sum, item) => sum + item[1], 0);
    const poorVitals = Object.entries(data.vitals).filter(([metric, value]) => value && vitalRating(metric, value) !== "good");
    const confidence = !analyticsAllowed() ? 0 : Math.min(100, Math.round(Math.log10(Math.max(1, data.rows.length)) * 34));
    const priorities = [
      !analyticsAllowed()
        ? { level: "warning", title: "Bật consent Analytics", note: "Chưa có quyền đo nên Decision Center không tạo kết luận từ hành vi.", tab: "realtime" }
        : null,
      errorCount > 0
        ? { level: "danger", title: `Kiểm tra ${errorCount} lỗi trình duyệt`, note: "Mở Quality để xem nhóm lỗi an toàn và vị trí cần ưu tiên.", tab: "quality" }
        : { level: "good", title: "Không có lỗi trình duyệt trong khoảng chọn", note: "Tiếp tục theo dõi cảnh báo bất thường và Web Vitals.", tab: "quality" },
      poorVitals.length
        ? { level: "warning", title: `Tối ưu ${poorVitals.map(([metric]) => metric.toUpperCase()).join(", ")}`, note: "Các chỉ số này đang ngoài ngưỡng tốt trong phiên hiện tại.", tab: "quality" }
        : null,
      visits > 0 && engagementRate < 45
        ? { level: "warning", title: "Giảm rơi trước tương tác", note: `Tỷ lệ có hành động hiện là ${engagementRate}%; kiểm tra top module và luồng vào.`, tab: "journey" }
        : null,
      visits > 0 && submits < actions
        ? { level: "info", title: "Rà soát hành trình form", note: `Từ ${actions} phiên tương tác có ${submits} phiên gửi form; xem funnel trước khi thay đổi giao diện.`, tab: "journey" }
        : null
    ].filter(Boolean).slice(0, 4);
    return `<div class="insights-analysis-grid insights-decision-grid"><section class="insights-analysis-card insights-wide insights-decision-queue"><header><div><small>DECISION QUEUE</small><h3>Việc nên xem tiếp theo</h3></div><span class="insights-confidence"><b>${confidence}%</b> độ tin cậy dữ liệu</span></header><div class="insights-priority-list">${priorities.map((item, index) => `<article class="${item.level}"><span>${String(index + 1).padStart(2, "0")}</span><div><strong>${escapeHtml(item.title)}</strong><p>${escapeHtml(item.note)}</p></div><button type="button" data-insights-tab="${item.tab}" aria-label="Mở báo cáo ${escapeHtml(item.title)}">Xem →</button></article>`).join("") || '<article class="good"><span>✓</span><div><strong>Chưa có ưu tiên mới</strong><p>Hệ thống chưa đủ dữ liệu để đề xuất bước tiếp theo.</p></div></article>'}</div></section><section class="insights-analysis-card"><small>JOURNEY SCORECARD</small><h3>Từ truy cập đến kết quả</h3><dl class="insights-decision-score"><div><dt>Tương tác</dt><dd>${engagementRate}%</dd></div><div><dt>Gửi form</dt><dd>${submitRate}%</dd></div><div><dt>Chuyển đổi</dt><dd>${conversionRate}%</dd></div></dl><button type="button" data-insights-tab="journey">Mở Funnel & Cohort</button></section><section class="insights-analysis-card"><small>DATA READINESS</small><h3>Dữ liệu đủ để quyết định?</h3><div class="insights-readiness" style="--readiness:${confidence}%"><i><b></b></i><strong>${confidence < 35 ? "Cần thêm tín hiệu" : confidence < 70 ? "Đủ để định hướng" : "Sẵn sàng phân tích"}</strong><p>${analyticsAllowed() ? `${formatNumber(data.rows.length)} sự kiện đã lọc theo schema công khai.` : "Consent Analytics chưa được bật."}</p></div></section></div>`;
  }
  ...
  function analyticsMarkup(range = "30m", tab = "decisions") {
    const data = aggregate(range); const tabs = [["decisions", "Quyết định"], ["realtime", "Realtime"], ["journey", "Funnel & Cohort"], ["quality", "Quality"], ["experiments", "A/B Test"]];
    const panel = tab === "decisions" ? decisionPanel(data) : tab === "journey" ? journeyPanel(data) : tab === "quality" ? qualityPanel(data) : tab === "experiments" ? experimentPanel(data) : realtimePanel(data, range === "5m" ? "5m" : "30m");
    return `<section class="insights-pro insights-analytics" data-insights-analytics data-range="${range}" data-tab="${tab}"><header class="insights-analytics-hero"><div><span class="insights-eyebrow"><i></i> ANALYTICS INTELLIGENCE</span><h2>Đọc đúng tín hiệu.<br><em>Hành động có lý do.</em></h2><p>Decision Queue, realtime 5/30 phút, funnel, retention, cohort, nguồn truy cập, heatmap, Web Vitals, A/B và cảnh báo — chỉ khi có consent.</p></div><div class="insights-score"><small>LOCAL EVENTS</small><strong>${data.rows.length}</strong><span>${analyticsAllowed() ? "Consent đã bật" : "Consent chưa bật"}</span></div></header><div class="insights-analytics-toolbar"><nav aria-label="Các chế độ Analytics">${tabs.map(([id, label]) => `<button type="button" data-insights-tab="${id}" class="${tab === id ? "active" : ""}" aria-pressed="${tab === id}">${label}</button>`).join("")}</nav><div><label class="insights-range-label"><span>Khoảng</span><select data-insights-range><option value="5m" ${range === "5m" ? "selected" : ""}>5 phút</option><option value="30m" ${range === "30m" ? "selected" : ""}>30 phút</option><option value="7d" ${range === "7d" ? "selected" : ""}>7 ngày</option><option value="30d" ${range === "30d" ? "selected" : ""}>30 ngày</option><option value="all" ${range === "all" ? "selected" : ""}>Tất cả</option></select></label><button type="button" data-insights-refresh>↻ Làm mới</button><button type="button" class="primary" data-insights-export="csv">Xuất CSV</button></div></div><section class="insights-metrics"><article><small>Phiên</small><strong>${formatNumber(data.sessions)}</strong><p>session ID ngẫu nhiên</p></article><article><small>Page views</small><strong>${formatNumber(data.views.length)}</strong><p>route đã lọc query</p></article><article><small>Actions</small><strong>${formatNumber(data.actions.length)}</strong><p>control công khai</p></article><article><small>Active</small><strong>${formatDuration(data.session.activeSeconds || 0)}</strong><p>phiên hiện tại</p></article></section><div data-insights-pane>${panel}</div></section>`;
  }
  function mountOverview(host) { if (host) host.innerHTML = overviewMarkup(); }
  function mountAnalytics(host) { if (!host) return; analyticsHost = host; host.innerHTML = analyticsMarkup("30m", "decisions"); clearInterval(liveTimer); liveTimer = setInterval(() => { const root = analyticsHost?.querySelector("[data-insights-analytics]"); if (root && !document.hidden && ["5m", "30m"].includes(root.dataset.range)) rerenderAnalytics(); }, 15000); }
  ...
  document.addEventListener("click", (event) => {
    ...
    const tab = event.target.closest("[data-insights-tab]"); if (tab && analyticsHost) { const root = analyticsHost.querySelector("[data-insights-analytics]"); analyticsHost.innerHTML = analyticsMarkup(root?.dataset.range || "30m", tab.dataset.insightsTab); return; }
