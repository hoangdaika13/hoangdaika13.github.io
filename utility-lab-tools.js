(function initHHUtilityLabTools(global) {
  ...
  async function runAction(root, tool, action) {
    ...
    else if (action === "network-measure") { runtime.controller = new AbortController(); const start = performance.now(), response = await fetch(`assets/brand/pwa-512.png?v=1&speed=${Date.now()}`, { cache: "no-store", signal: runtime.controller.signal }); const blob = await response.blob(), elapsed = Math.max(1, performance.now() - start), mbps = blob.size * 8 / elapsed / 1000; setOutput(root, `Tệp thử: ${(blob.size / 1024).toFixed(1)} KB\nThời gian: ${elapsed.toFixed(0)} ms\nThroughput phiên: ${mbps.toFixed(2)} Mbps\nNetwork Information: ${navigator.connection?.effectiveType || "không hỗ trợ"}`); }
