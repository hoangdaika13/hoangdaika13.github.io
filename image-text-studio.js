(function initHHImageTextStudio(global) {
  ...
  function snapshotState() {
    return JSON.stringify({
      ...
      overrides: state.items.map((item) => [item.name, item.overrides, item.focusX, item.focusY])
    });
  }
    ...
    root.innerHTML = `
      ...
      <footer class="its-exportbar">
        ...
        <div class="its-export-actions">
          ...
          <button type="button" data-action="export-zip">Tải ZIP đã chọn</button>
          <button type="button" class="is-primary" data-action="export-folder">Xuất toàn bộ vào thư mục</button>
        </div>
      </footer>
  ...
  function renderLibrary() {
    ...
    if (grid) {
      grid.innerHTML = items.map((item) => {
        ...
        return `<article class="its-thumb${item.id === state.activeId ? " is-active" : ""}" data-image-id="${item.id}" title="${escapeHtml(item.name)}">
  ...
  function schedulePreview() {
    ...
    Promise.resolve(drawComposite(previewContext, width, height, item, { preview: true }))
      .then(() => {
  ...
  function applyPreset(id) {
    ...
    if (!preset) return;
    pushHistory();
    state.template = clone({ title: preset.title, subtitle: preset.subtitle, footer: preset.footer });
  ...
  function setSetting(property, rawValue) {
    ...
    if (["safeZone"].includes(property)) value = Boolean(rawValue);
  ...
  function projectData() {
    return {
      ...
      template: state.template,
      settings: state.settings,
      images: state.items.map((item) => ({ name: item.name, overrides: item.overrides, focusX: item.focusX, focusY: item.focusY }))
    };
  }
  ...
  function outputName(item) {
    return `${sanitizeName(item.name)}${sanitizeName(state.settings.suffix || "-thumbnail")}.${outputExtension()}`;
  }
  ...
  function bindEvents() {
    ...
    root.addEventListener("focusin", (event) => {
      if (event.target.matches("[data-layer-prop],[data-setting]")) beforeEditSnapshot = snapshotState();
    });
    ...
    root.addEventListener("click", (event) => {
      ...
      else if (name === "export-folder") exportFolder();
