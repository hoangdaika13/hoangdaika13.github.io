(function initHHSchoolOffline(root) {
  ...
  const api = Object.freeze({
    ...
    async enqueue(ownerId, learnerProfileId, request) {
      const current = await api.listQueue(); const ownerProfile = profileKey(ownerId, learnerProfileId); const sameProfile = current.filter((item) => item.ownerProfile === ownerProfile);
      const isProgressSave = request?.url === "/api/education/progress" && String(request?.method || "PUT").toUpperCase() === "PUT";
      const superseded = isProgressSave ? sameProfile.filter((item) => item.request?.url === request.url && String(item.request?.method || "PUT").toUpperCase() === "PUT" && item.syncStatus !== "needs-resolution") : [];
      await Promise.all(superseded.map((item) => api.removeQueue(item.id)));
      if (sameProfile.length - superseded.length >= 20) throw Object.assign(new Error("Hàng đợi đồng bộ đã đạt giới hạn 20 tác vụ."), { code: "SYNC_QUEUE_LIMIT" });
      const item = { id: `sync-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`, ownerProfile, ownerId, learnerProfileId, request, attempts: 0, maxAttempts: 5, syncStatus: "queued", createdAt: Date.now() }; await transaction(STORES.queue, "readwrite", (store) => store.put(item)); try { await root.navigator?.serviceWorker?.ready?.then((registration) => registration.sync?.register?.("hh-school-progress")); } catch {} return item;
    },
    ...
    async clearConflicts(ownerId, learnerProfileId) { const ownerProfile = profileKey(ownerId, learnerProfileId); const items = await api.listQueue(); const conflicts = items.filter((item) => item.ownerProfile === ownerProfile && item.syncStatus === "needs-resolution"); await Promise.all(conflicts.map((item) => api.removeQueue(item.id))); return conflicts.length; },
    async flush(sender) { if (root.document?.hidden) return []; const items = await api.listQueue(); const results = []; for (const item of items) { if (item.syncStatus === "needs-resolution") { results.push({ id: item.id, ok: false, conflict: true, error: item.lastError || "Cần giải quyết xung đột đồng bộ." }); continue; } if (item.attempts >= (item.maxAttempts || 5)) { results.push({ id: item.id, ok: false, terminal: true, error: "Đã vượt số lần thử đồng bộ." }); continue; } try { await sender(item.request); await api.removeQueue(item.id); results.push({ id: item.id, ok: true }); } catch (error) { const conflict = Number(error?.status) === 409; await transaction(STORES.queue, "readwrite", (store) => store.put({ ...item, attempts: conflict ? item.attempts : item.attempts + 1, syncStatus: conflict ? "needs-resolution" : "queued", conflict: conflict ? error.data?.conflict || null : item.conflict, lastError: error.message, lastAttemptAt: Date.now() })); results.push({ id: item.id, ok: false, conflict, error: error.message }); } } return results; }
  });
