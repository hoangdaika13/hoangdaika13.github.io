(function facebookPageCommandCenter(global) {
  ...
  const TABS = [
    ...
    ["campaigns", "Chiến dịch"], ["library", "Thư viện"], ["calendar", "Lịch"], ["comments", "Bình luận"], ["insights", "Insights"],
  ...
  let comments = [];
  let insights = [];
  let plannerRecommendation = null;
  let busy = "";
  let toastTimer = 0;
  let setupEdit = null;
  ...
  function activeTemplate() { return statusData?.templates?.find((item) => item.id === state.templateId) || null; }
  ...
  function composerViewV2() {
    ...
    const template = campaign ? null : activeTemplate();
    const source = campaign || template || {};
    const templateMessage = state.templateVariant === "a" ? template?.variantA : state.templateVariant === "b" ? template?.variantB : template?.message;
    ...
    return `${heading(campaign ? `Sửa · ${esc(campaign.name)}` : template ? `Từ mẫu · ${esc(template.name)}` : "Composer đa Page", "Tạo nội dung chung hoặc ghi đè riêng cho từng Page, sau đó lưu nháp, gửi duyệt hoặc đăng trực tiếp.", campaign || template ? `<button class="fpc-btn" type="button" data-fpc-new-campaign>Chiến dịch mới</button>` : "")}
      <form class="fpc-card fpc-card--full" data-fpc-compose-form><input type="hidden" name="id" value="${esc(campaign?.id || "")}">
        <div class="fpc-form-grid"><label class="fpc-field"><span>Tên chiến dịch</span><input name="name" value="${esc(campaign?.name || (template ? `${template.name} · Campaign` : ""))}" placeholder="Chiến dịch tháng này"></label><label class="fpc-field"><span>Mục tiêu</span><select name="objective"><option value="engagement">Tương tác</option><option value="reach" ${campaign?.objective === "reach" ? "selected" : ""}>Tiếp cận</option><option value="traffic" ${campaign?.objective === "traffic" ? "selected" : ""}>Lưu lượng</option><option value="community" ${campaign?.objective === "community" ? "selected" : ""}>Cộng đồng</option></select></label>
          ...
          <label class="fpc-field fpc-field--full"><span>Nội dung chung</span><textarea name="message" maxlength="63206" placeholder="Viết nội dung thật sẽ đăng lên Facebook…" required>${esc(template ? (templateMessage || template.message || "") : campaign?.message || "")}</textarea></label>
          <label class="fpc-field"><span>Loại nội dung</span><select name="mediaType"><option value="text">Văn bản / liên kết</option><option value="photo" ${source.mediaType === "photo" ? "selected" : ""}>Ảnh từ URL</option><option value="video" ${source.mediaType === "video" ? "selected" : ""}>Video từ URL</option></select></label><label class="fpc-field"><span>Tags nội bộ</span><input name="tags" value="${esc((source.tags || []).join(", "))}" placeholder="sale, tháng 8"></label>
          <label class="fpc-field"><span>Link đính kèm</span><input name="link" type="url" value="${esc(source.link || "")}" placeholder="https://…"></label><label class="fpc-field"><span>URL ảnh hoặc video</span><input name="mediaUrl" type="url" value="${esc(source.mediaUrl || "")}" placeholder="https://…"></label>
  ...
  function campaignsView() {
    ...
    const approvalInbox = `<div class="fpc-grid" style="margin-top:11px">${campaigns.filter((item) => ["review", "approved"].includes(item.status)).map((item) => `<article class="fpc-card fpc-card--wide"><h3>Hộp duyệt · ${esc(item.name)}</h3><div class="fpc-list">${(item.approvalNotes || []).slice(-5).map((note) => `<div class="fpc-list-item"><i class="fpc-task-dot ${note.kind === "request_changes" ? "is-error" : ""}"></i><div><strong>${esc(note.authorName || "Reviewer")} · ${esc(note.kind)}</strong><span>${esc(note.message)}</span><small>${formatDate(note.createdAt)}</small></div></div>`).join("") || `<div class="fpc-empty"><p>Chưa có ghi chú duyệt.</p></div>`}</div><form class="fpc-actions" data-fpc-campaign-note="${esc(item.id)}"><input class="fpc-page-search" style="flex:1" name="message" maxlength="1200" placeholder="Ghi chú, yêu cầu sửa hoặc lý do duyệt…" required><select class="fpc-page-search" style="width:auto" name="kind"><option value="comment">Ghi chú</option><option value="request_changes">Yêu cầu sửa</option></select><button class="fpc-btn" type="submit">Gửi ghi chú</button></form></article>`).join("") || `<article class="fpc-card fpc-card--full"><div class="fpc-empty"><p>Chiến dịch gửi duyệt sẽ xuất hiện trong Approval Inbox.</p></div></article>`}</div>`;
    const summary = `${heading("Campaign & Approval Center", "Quản lý chiến dịch từ nháp đến xuất bản, giữ lịch sử duyệt và kết quả từng Page.", `<button class="fpc-btn fpc-btn--primary" type="button" data-fpc-new-campaign>Tạo chiến dịch</button>`)}
      ...
      <article class="fpc-card fpc-card--full"><div class="fpc-table-wrap"><table class="fpc-table"><thead><tr><th>Chiến dịch</th><th>Page</th><th>Trạng thái</th><th>Lịch</th><th>Cập nhật</th><th>Thao tác</th></tr></thead><tbody>${campaigns.map((item) => `<tr><td><strong>${esc(item.name)}</strong><br><small>${esc(item.objective || "—")} · ${(item.approvalNotes || []).length} ghi chú</small></td><td>${item.pageIds.length}</td><td><span class="fpc-badge">${esc(item.status)}</span></td><td>${item.scheduledAt ? formatDate(item.scheduledAt) : "Đăng ngay"}</td><td>${formatDate(item.updatedAt)}</td><td><button class="fpc-btn" type="button" data-fpc-edit-campaign="${esc(item.id)}">Mở</button> ${item.status === "review" ? `<button class="fpc-btn" type="button" data-fpc-stage-campaign="${esc(item.id)}" data-status="approved">Duyệt</button><button class="fpc-btn" type="button" data-fpc-stage-campaign="${esc(item.id)}" data-status="draft">Trả về</button>` : ""} ${item.status === "approved" ? `<button class="fpc-btn fpc-btn--primary" type="button" data-fpc-publish-campaign="${esc(item.id)}">Xuất bản</button>` : ""} ${["draft", "review", "approved", "failed", "partial"].includes(item.status) ? `<button class="fpc-btn fpc-btn--danger" type="button" data-fpc-delete-campaign="${esc(item.id)}">Xóa</button>` : ""}</td></tr>`).join("") || `<tr><td colspan="6">Chưa có chiến dịch.</td></tr>`}</tbody></table></div></article></div>`;
    return `${summary}${approvalInbox}`;
  }

  function libraryView() {
    const templates = statusData?.templates || [];
    return `${heading("Content Library & A/B Lab", "Lưu mẫu dùng lại cho nhiều Page, chuẩn bị hai biến thể nội dung và đưa thẳng vào Composer.")}
      <div class="fpc-grid"><form class="fpc-card fpc-card--wide" data-fpc-template-form><h3>Tạo mẫu nội dung</h3><div class="fpc-form-grid"><label class="fpc-field"><span>Tên mẫu</span><input name="name" required placeholder="Ra mắt video mới"></label><label class="fpc-field"><span>Nhóm</span><input name="category" placeholder="video, sale, community"></label><label class="fpc-field fpc-field--full"><span>Nội dung chính</span><textarea name="message" placeholder="Nội dung mặc định…"></textarea></label><label class="fpc-field"><span>Biến thể A</span><textarea name="variantA" placeholder="Hook ngắn, trực tiếp…"></textarea></label><label class="fpc-field"><span>Biến thể B</span><textarea name="variantB" placeholder="Hook kể chuyện…"></textarea></label><label class="fpc-field"><span>Loại media</span><select name="mediaType"><option value="text">Văn bản/link</option><option value="photo">Ảnh</option><option value="video">Video</option></select></label><label class="fpc-field"><span>Tags</span><input name="tags" placeholder="video, tuần 32"></label><label class="fpc-field"><span>Link</span><input type="url" name="link" placeholder="https://…"></label><label class="fpc-field"><span>Media URL</span><input type="url" name="mediaUrl" placeholder="https://…"></label></div><div class="fpc-actions"><button class="fpc-btn fpc-btn--primary" type="submit">Lưu vào thư viện</button></div></form><article class="fpc-card"><h3>Nguyên tắc A/B</h3><div class="fpc-notice">A/B trong tool tạo biến thể nội dung để bạn xuất bản có kiểm soát. Tool không tự tuyên bố biến thể thắng nếu chưa có đủ dữ liệu bài đăng thật.</div><p style="color:#8093ab;font-size:10px;line-height:1.5">Chỉ thay đổi hook hoặc lời kêu gọi hành động giữa A và B để kết quả dễ so sánh.</p></article></div>
      <div class="fpc-grid" style="margin-top:11px">${templates.map((item) => `<article class="fpc-card"><div class="fpc-section-title"><strong>${esc(item.category)}</strong><small>${item.useCount || 0} lần dùng</small></div><h3>${esc(item.name)}</h3><p class="fpc-clamp">${esc(item.message || item.variantA || item.variantB)}</p><div class="fpc-actions"><button class="fpc-btn fpc-btn--primary" type="button" data-fpc-use-template="${esc(item.id)}" data-variant="main">Dùng mẫu</button>${item.variantA ? `<button class="fpc-btn" type="button" data-fpc-use-template="${esc(item.id)}" data-variant="a">Dùng A</button>` : ""}${item.variantB ? `<button class="fpc-btn" type="button" data-fpc-use-template="${esc(item.id)}" data-variant="b">Dùng B</button>` : ""}<button class="fpc-btn fpc-btn--danger" type="button" data-fpc-delete-template="${esc(item.id)}">Xóa</button></div></article>`).join("") || `<article class="fpc-card fpc-card--full"><div class="fpc-empty"><p>Chưa có mẫu nội dung.</p></div></article>`}</div>`;
  }
  ...
  function calendarView() {
    ...
    return `${heading("Lịch nội dung & Smart Planner", "Theo dõi bài đã lên lịch và phân tích giờ đăng từ chính hiệu suất bài thật của Page.", `<div class="fpc-actions"><button class="fpc-btn" type="button" data-fpc-analyze-planner>Phân tích giờ đăng</button><button class="fpc-btn fpc-btn--primary" type="button" data-fpc-tab="composer">Lên lịch bài</button></div>`)}
      <div class="fpc-card fpc-card--full" style="margin-bottom:11px"><h3>Giờ đăng đề xuất · UTC+7</h3>${plannerRecommendation?.ready ? `<div class="fpc-slot-grid">${plannerRecommendation.slots.map((slot) => `<div class="fpc-slot"><strong>${dayNames[slot.day]} · ${String(slot.hour).padStart(2, "0")}:00</strong><span>${slot.samples} bài · tương tác TB ${formatNumber(slot.averageEngagement)}</span><small>Độ tin cậy ${esc(slot.confidence)}</small></div>`).join("")}</div><div class="fpc-notice" style="margin-top:9px">Phân tích ${plannerRecommendation.sampleSize} bài thật; đây là gợi ý thống kê, không đảm bảo kết quả tương lai.</div>` : `<div class="fpc-empty"><p>${plannerRecommendation?.reason === "insufficient-data" ? `Chỉ có ${plannerRecommendation.sampleSize} bài hợp lệ; cần tối thiểu 3 bài để phân tích.` : "Bấm Phân tích giờ đăng sau khi kết nối Page."}</p></div>`}</div>
  ...
  function workspace() {
    ...
    const views = { dashboard: dashboardView, pages: pagesViewV2, composer: composerViewV2, campaigns: campaignsView, library: libraryView, calendar: calendarView, comments: commentsView, insights: insightsView, automation: automationView, setups: setupsView, settings: settingsView };
  ...
  async function onClick(event) {
    ...
    if (event.target.closest("[data-fpc-new-campaign]")) { state.campaignEditId = ""; state.templateId = ""; state.templateVariant = "main"; state.tab = "composer"; saveState(); render(); return; }
    const useTemplate = event.target.closest("[data-fpc-use-template]");
    if (useTemplate) { state.campaignEditId = ""; state.templateId = useTemplate.dataset.fpcUseTemplate; state.templateVariant = useTemplate.dataset.variant || "main"; state.tab = "composer"; saveState(); await api("templates/use", "POST", { id: state.templateId }).then(() => refresh(false)).catch((error) => { render(); notify(error.message, "error"); }); return; }
    const deleteTemplateId = event.target.closest("[data-fpc-delete-template]")?.dataset.fpcDeleteTemplate;
    if (deleteTemplateId && confirm("Xóa mẫu nội dung này?")) { try { await api("templates/delete", "DELETE", { id: deleteTemplateId }); if (state.templateId === deleteTemplateId) state.templateId = ""; await refresh(false); notify("Đã xóa mẫu nội dung."); } catch (error) { notify(error.message, "error"); } return; }
    if (event.target.closest("[data-fpc-analyze-planner]")) { if (!state.activePageId) return notify("Hãy kết nối và chọn một Page.", "error"); busy = "planner"; render(); try { const result = await api("planner/recommendations", "GET", null, { pageId: state.activePageId, timezoneOffset: 7 }); plannerRecommendation = result.recommendation || null; } catch (error) { plannerRecommendation = null; notify(error.message, "error"); } busy = ""; render(); return; }
    ...
    if (editCampaignId) { const item = statusData?.campaigns?.find((campaign) => campaign.id === editCampaignId); state.campaignEditId = editCampaignId; state.templateId = ""; state.selectedPageIds = item?.pageIds || []; state.tab = "composer"; saveState(); render(); return; }
  ...
  async function onSubmitV2(event) {
    ...
    const template = event.target.closest("[data-fpc-template-form]");
    if (template) { event.preventDefault(); const data = new FormData(template); try { await api("templates/save", "POST", { name: data.get("name"), category: data.get("category"), message: data.get("message"), variantA: data.get("variantA"), variantB: data.get("variantB"), mediaType: data.get("mediaType"), tags: data.get("tags"), link: data.get("link"), mediaUrl: data.get("mediaUrl") }); template.reset(); await refresh(false); notify("Đã lưu mẫu vào Content Library."); } catch (error) { notify(error.message, "error"); } return; }
    const campaignNote = event.target.closest("[data-fpc-campaign-note]");
    if (campaignNote) { event.preventDefault(); const data = new FormData(campaignNote); try { await api("campaigns/note", "POST", { id: campaignNote.dataset.fpcCampaignNote, message: data.get("message"), kind: data.get("kind") }); campaignNote.reset(); await refresh(false); notify("Đã lưu ghi chú duyệt."); } catch (error) { notify(error.message, "error"); } return; }
