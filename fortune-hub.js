(function fortuneHubModule(globalScope, factory) {
  ...
  async function handleClick(runtime, event) {
    ...
    if (event.target.closest("[data-fortune-symbol-draw]")) {
      ...
      runtime.session.symbolDeck = v4?.drawSymbolDeck?.(runtime.session.symbolType, runtime.session.symbolSeed, runtime.session.symbolCount, { allowReversed: runtime.session.runeAllowReversed }) || null; runtime.session.symbolSeed = runtime.session.symbolDeck?.seed || runtime.session.symbolSeed;
    ...
    if (event.target.closest("[data-fortune-academy-next]")) { runtime.session.academyRound = Math.max(1, Number(runtime.session.academyRound || 1) + 1); runtime.session.academyMode = runtime.root.querySelector("[data-fortune-academy-mode]")?.value || runtime.session.academyMode || "meaning"; runtime.session.tarotQuiz = suiteV4()?.tarotQuiz?.(`academy-${runtime.session.academyRound}`, { mode: runtime.session.academyMode }) || null; runtime.session.academyFeedback = "Hãy tự chọn trước khi mở đáp án."; render(runtime, true); return; }
    ...
    if (academyAnswer) {
      ...
      const selected = Number(academyAnswer.dataset.fortuneAcademyAnswer); const correct = selected === quiz.correctIndex; runtime.session.academyFeedback = correct ? `✓ Chính xác: ${quiz.answers[quiz.correctIndex]}` : `Chưa đúng. Sau khi đã nộp, đáp án được mở: ${quiz.answers[quiz.correctIndex]}`; runtime.session.academyReview = suiteV4()?.academyReviewSchedule?.(correct ? 4 : 1, runtime.session.academyReview || {}) || null; render(runtime, true); return;
    }
