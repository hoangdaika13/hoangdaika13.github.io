(function fortuneHubModule(globalScope, factory) {
  ...
  function academyV4Markup(runtime) {
    ...
    const quizMarkup = quiz ? "<div class=\"fortune-academy-card\">" + (quiz.card.image ? "<img src=\"" + escapeHtml(quiz.card.image) + "\" alt=\"" + escapeHtml(quiz.card.name) + "\" width=\"360\" height=\"617\" loading=\"lazy\" decoding=\"async\">" : "<i>" + quiz.card.symbol + "</i>") + "<strong>" + escapeHtml(quiz.card.name) + "</strong><p>Điều nào mô tả lớp ánh sáng của lá này theo bộ nội dung HH?</p><div>" + quiz.answers.map((answer, index) => "<button type=\"button\" data-fortune-academy-answer=\"" + index + "\">" + escapeHtml(answer) + "</button>").join("") + "</div><small data-fortune-academy-feedback>" + escapeHtml(runtime.session.academyFeedback || "Hãy tự chọn trước khi mở đáp án.") + "</small></div>" : "";
  ...
  function buildCopilotFactLock(runtime, input) {
    ...
    else if (view === "iching" && runtime.session.iching) { add("iching.primary", "symbolic", runtime.session.iching.title, runtime.session.iching.title); add("iching.nuclear", "symbolic", runtime.session.iching.nuclearTitle, runtime.session.iching.nuclearTitle); add("iching.changed", "symbolic", runtime.session.iching.changedTitle, runtime.session.iching.changedTitle); add("iching.lines", "calculation", `Hào động: ${runtime.session.iching.changing.join(", ") || "không có"}`); }
    else if (view === "numerology" && runtime.session.numerology) { add("numerology.life-path", "calculation", `Đường đời ${runtime.session.numerology.lifePath}: ${runtime.session.numerology.formula}`, `Đường đời ${runtime.session.numerology.lifePath}`); if (runtime.session.cycles) add("numerology.cycle", "calculation", runtime.session.cycles.formula); }
  ...
  async function handleClick(runtime, event) {
    ...
    if (event.target.closest("[data-fortune-astrology-calc]")) {
      ...
      if (!runtime.session.astrologyV4?.ok) showToast(runtime, runtime.session.astrologyV4?.errors?.[0] || "Không đủ dữ liệu để tính.", "error"); else { runtime.session.calculationCertificate = accuracyLab()?.createCalculationCertificate?.({ profile: runtime.profile, result: runtime.session.astrologyV4, provenance: runtime.session.astrologyV4.provenance || {} }) || null; playFortuneTone(runtime); showToast(runtime, "Đã tính xong, tạo provenance và Calculation Certificate SHA-256."); }
