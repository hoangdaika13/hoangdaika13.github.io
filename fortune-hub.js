(function fortuneHubModule(globalScope, factory) {
  ...
  const METHOD_CATALOG = Object.freeze([
    ...
    ,{ id: "symbols", title: "Lenormand · Rune · Oracle", system: "36 Lenormand có inset, 24 Elder Futhark có transliteration, 24 Oracle HH", input: "Bộ + số lá + seed + tùy chọn Rune đảo", algorithm: "Web Crypto seed + Fisher-Yates/Mulberry32 replay", precision: "Tái tạo chính xác và xác minh seed proof", nature: "Dữ liệu lịch sử/ngôn ngữ tách khỏi diễn giải biểu tượng", source: "British Museum · Unicode Runic · HH Original", version: "symbol-decks-v2" }
  ]);
  ...
  function navMarkup(view) {
    const groups = [
      ...
      ["HỆ BIỂU TƯỢNG", [["zodiac","☼","Cung & con giáp"],["numerology","#","Thần số học"],["iching","☯","Kinh Dịch 64 quẻ"]]],
      ["THIÊN VĂN & LỊCH", [["moon","☾","Mặt Trăng"],["sky","✺","Moon & Sky"],["chart","◎","Astrology Studio"],["eastern","☯","Hệ phương Đông"]]],
      ["NHẬT KÝ & KIỂM CHỨNG", [["compatibility","∞","Tương tác"],["calendar","▦","Lịch chiêm nghiệm"],["journal","✎","Nhật ký"],["copilot","AI","Reflection Copilot"],["accuracy","✓","Accuracy Lab"],["methods","ⓘ","Phương pháp"],["history","◷","Lịch sử"]]]
    ];
  ...
  function automaticAiMarkup(runtime, kind, localFallback = "") {
    ...
    const title = kind === "numerology" ? "Gemini · luận giải thần số" : "Gemini · luận giải cấu trúc quẻ";
  ...
  function ichingMarkup(runtime) {
    ...
    return `${toolbarMarkup("Kinh Dịch 64 · Professional Chart", "Quẻ chính, hỗ, biến, đối, đảo; sổ sáu hào và Gemini tự phân tích dữ kiện quẻ đã ẩn danh.")}<section class="fortune-iching fortune-iching-pro"><div class="fortune-calc-card"><header><i>☯</i><div><small>COINS · YARROW · MANUAL</small><h3>Gieo từ hào 1 lên hào 6</h3></div></header><label><span>Câu hỏi riêng (chỉ ở tab này, không gửi Gemini)</span><textarea rows="3" maxlength="240" data-fortune-iching-question placeholder="Viết ngắn gọn điều bạn đang cân nhắc...">${escapeHtml(runtime.session.ichingQuestion || "")}</textarea></label><div class="fortune-inline-fields"><label><span>Phương pháp</span><select data-fortune-iching-mode><option value="coins"${runtime.session.ichingMode === "coins" ? " selected" : ""}>Ba đồng xu</option><option value="yarrow"${runtime.session.ichingMode === "yarrow" ? " selected" : ""}>Xác suất cỏ thi</option><option value="manual"${runtime.session.ichingMode === "manual" ? " selected" : ""}>Nhập 6 hào</option></select></label><label><span>Seed tái tạo</span><input type="text" maxlength="120" data-fortune-iching-seed value="${escapeHtml(runtime.session.ichingSeed || "")}" placeholder="Để trống để tạo mới"></label></div><div class="fortune-v4-inline-fields fortune-iching-manual">${[0,1,2,3,4,5].map((index) => `<label><span>Hào ${index + 1}</span><select data-fortune-iching-manual>${[6,7,8,9].map((value)=>`<option value="${value}"${runtime.session.ichingManual?.[index] === value ? " selected" : ""}>${value} · ${value===6?"âm động":value===7?"dương tĩnh":value===8?"âm tĩnh":"dương động"}</option>`).join("")}</select></label>`).join("")}</div><button class="fortune-primary" type="button" data-fortune-iching-cast>Gieo quẻ & tự phân tích bằng Gemini</button><p class="fortune-ai-privacy">Gemini chỉ nhận số quẻ, tên quẻ, quái, giá trị 6 hào và thứ tự hào động; không nhận câu hỏi riêng hoặc seed.</p></div>${chart}${automaticAiMarkup(runtime, "iching", result ? `${result.title}. ${result.reflection} ${result.question}` : "Kết quả và diễn giải HH cục bộ không phụ thuộc Gemini.")}</section>`;
  }
  ...
  async function runAutomaticFortuneAi(runtime, kind) {
    if (!["numerology", "iching"].includes(kind)) return;
  ...
  async function handleClick(runtime, event) {
    ...
    if (symbolFocus) { runtime.session.symbolFocusIndex = clamp(symbolFocus.dataset.fortuneSymbolFocus, 0, Math.max(0, (runtime.session.symbolDeck?.cards?.length || 1) - 1), 0); render(runtime, true); return; }
    ...
    if (event.target.closest("[data-fortune-astrology-calc]")) {
      ...
      if (!runtime.session.astrologyV4?.ok) showToast(runtime, runtime.session.astrologyV4?.errors?.[0] || "Không đủ dữ liệu để tính.", "error"); else { runtime.session.astrologyV4 = attachAstrologyResultContract(runtime.session.astrologyV4, runtime.profile, mode, targetUtc); runtime.session.calculationCertificate = accuracyLab()?.createCalculationCertificate?.({ profile: runtime.profile, result: runtime.session.astrologyV4, provenance: runtime.session.astrologyV4.provenance || {} }) || null; playFortuneTone(runtime); showToast(runtime, "Đã tính xong, tạo Result Contract và Calculation Certificate SHA-256."); }
      render(runtime, true); return;
    }
    ...
    if (event.target.closest("[data-fortune-zodiac-calc]")) {
      ...
      addHistory(runtime, "zodiac", runtime.session.western.sign.name, `Mặt Trời ${runtime.session.western.sign.longitude}° · ${runtime.session.western.nearBoundary ? "gần ranh giới" : "không gần ranh giới"}.`); render(runtime, true); return;
    }
    if (event.target.closest("[data-fortune-chinese-calc]")) {
      ...
      addHistory(runtime, "chinese", `${runtime.session.chinese.animal} · ${runtime.session.chinese.branch}`, `${runtime.session.chinese.yinYang} ${runtime.session.chinese.element} · mốc ${runtime.session.chinese.boundaryLabel}.`); render(runtime, true); return;
    }
    ...
    if (event.target.closest("[data-fortune-iching-cast]")) {
      ...
      runtime.session.ichingAi = null; playFortuneTone(runtime); addHistory(runtime, "iching", runtime.session.iching.title, `${runtime.session.iching.nuclearTitle} · ${runtime.session.iching.changedTitle} · hào động ${runtime.session.iching.changing.join(", ") || "không có"}.`); await runAutomaticFortuneAi(runtime, "iching"); return;
    }
