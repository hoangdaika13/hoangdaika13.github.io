(function fortuneHubModule(globalScope, factory) {
  ...
  const VIEWS = new Set(["today", "profile", "session", "accuracy", "tarot", "academy", "zodiac", "numerology", "iching", "moon", "sky", "calendar", "chart", "eastern", "symbols", "compatibility", "journal", "copilot", "methods", "history"]);
  ...
  const METHOD_CATALOG = Object.freeze([
    { id: "tarot", title: "Tarot HH 78", system: "22 Major + 56 Minor · trải 1/3/5/7/10/12/15 lá", input: "Chủ đề tùy chọn + seed", algorithm: "Fisher-Yates + Mulberry32 từ FNV-1a seed; lá đảo 36%", precision: "Tái tạo chính xác theo cùng seed, số lá và chế độ đảo", nature: "Diễn giải biểu tượng", source: "Nội dung và ký hiệu HH tự biên soạn", version: "tarot-78-v1" },
    ...
    ,{ id: "sky", title: "Moon & Sky", system: "Pha, chiếu sáng, khoảng cách, mọc/lặn, apsis, eclipse và mùa", input: "Ngày + IANA timezone + tọa độ + độ cao", algorithm: "Astronomy Engine VSOP87/NOVAS", precision: "Phụ thuộc mô hình, vị trí và đường chân trời; không dùng cho điều hướng", nature: "Tính toán thiên văn", source: "Astronomy Engine MIT · USNO definitions", version: "moon-sky-v1" }
    ,{ id: "eastern", title: "Nền lịch phương Đông", system: "Can Chi năm + Intl Chinese Calendar + 24 tiết khí", input: "Ngày + IANA timezone", algorithm: "Chu kỳ Can Chi và tìm kinh độ Mặt Trời", precision: "Bát Tự/Tử Vi chưa bật khi chưa qua fixture và chuyên gia", nature: "Lịch tính toán + biểu tượng có giới hạn", source: "HH local engine + Astronomy Engine", version: "eastern-calendar-v1" }
    ,{ id: "symbols", title: "Lenormand · Rune · Oracle", system: "36 Lenormand, 24 Elder Futhark, 24 Oracle HH", input: "Bộ + số lá + seed", algorithm: "Fisher-Yates/Mulberry32", precision: "Tái tạo chính xác theo seed", nature: "Diễn giải biểu tượng", source: "Nội dung HH tự biên soạn", version: "symbol-decks-v1" }
  ]);
  ...
  function sanitizeProfile(input, remembered = false) {
    ...
    return {
      ...
      zodiacMode: profile.zodiacMode === "sidereal" ? "sidereal" : "tropical", houseSystem: profile.houseSystem === "whole-sign" ? "whole-sign" : "equal",
  ...
  function navMarkup(view) {
    const groups = [
      ["BẮT ĐẦU", [["today","✦","Hôm nay"],["profile","◉","Hồ sơ phiên"],["session","➜","Phiên tổng hợp"],["accuracy","✓","Provenance Center"]]],
      ["CÔNG CỤ", [["tarot","♢","Tarot 78 Studio"],["academy","▣","Tarot Academy"],["zodiac","☼","Cung & con giáp"],["numerology","#","Thần số học"],["iching","☯","Kinh Dịch 64 quẻ"],["moon","☾","Mặt Trăng"],["sky","✺","Moon & Sky"],["chart","◎","Astrology Studio"],["eastern","☯","Hệ phương Đông"],["symbols","✧","Lenormand · Rune"]]],
      ["THEO DÕI", [["compatibility","∞","Tương tác"],["calendar","▦","Lịch chiêm nghiệm"],["journal","✎","Nhật ký"],["copilot","AI","Reflection Copilot"],["methods","ⓘ","Phương pháp"],["history","◷","Lịch sử"]]]
    ];
  ...
  function todayMarkup(runtime) {
    ...
    return `${toolbarMarkup("Lời nhắc cho hôm nay", "Một gợi ý ổn định theo ngày để tự quan sát, không phải dự đoán tương lai.")}
      <section class="fortune-today-grid">
        ...
        <article class="fortune-path-card"><header><span>MỞ NHANH</span><strong>18 không gian chuyên biệt</strong></header><div>${[["profile","◉","Hồ sơ phiên"],["accuracy","✓","Provenance"],["session","➜","Phiên tổng hợp"],["tarot","♢","Tarot 78"],["academy","▣","Tarot Academy"],["numerology","#","Con số & chu kỳ"],["iching","☯","Kinh Dịch 64 quẻ"],["moon","☾","Mặt Trăng"],["sky","✺","Moon & Sky"],["chart","◎","Astrology Studio"],["eastern","☯","Hệ phương Đông"],["symbols","✧","Lenormand · Rune"],["calendar","▦","Lịch chiêm nghiệm"],["compatibility","∞","Tương tác"],["journal","✎","Nhật ký mã hóa"],["copilot","AI","Reflection Copilot"],["methods","ⓘ","Phương pháp"],["history","◷","Lịch sử"]].map(([view, icon, label]) => `<button type="button" data-fortune-view="${view}"><i>${icon}</i><span>${label}</span><b>→</b></button>`).join("")}</div></article>
  ...
  function tarotMarkup(runtime) {
    ...
    if (cards.length) results += explanationMarkup(runtime, "Cách đọc trải bài này", [
      ...
      { label: "Cách chọn lá", text: "Bộ Tarot HH 78 lá (22 Major + 56 Minor) được xáo bằng bộ sinh số có seed; mỗi lá chỉ xuất hiện một lần trong cùng trải bài." },
    ...
    return `${toolbarMarkup("Tarot 78 Studio", "Đủ 78 lá Major và Minor do HH biên soạn; kéo đổi vị trí, ghim, ghi chú, so sánh và xuất nhiều tỷ lệ.")}<section class="fortune-control-panel"><label><span>Chủ đề hoặc câu hỏi (không lưu)</span><input type="text" maxlength="180" data-fortune-tarot-question value="${escapeHtml(runtime.session.question || "")}" placeholder="Bạn muốn soi chiếu điều gì?"></label><label><span>Kiểu trải bài</span><select data-fortune-tarot-count><option value="1"${runtime.session.tarotCount === 1 ? " selected" : ""}>1 lá · Trọng tâm</option><option value="3"${runtime.session.tarotCount === 3 ? " selected" : ""}>3 lá · Bối cảnh / Chú ý / Bước thử</option><option value="5"${runtime.session.tarotCount === 5 ? " selected" : ""}>5 lá · Toàn cảnh</option><option value="7"${runtime.session.tarotCount === 7 ? " selected" : ""}>7 lá · Hành trình</option><option value="10"${runtime.session.tarotCount === 10 ? " selected" : ""}>10 lá · Tổng hợp chuyên sâu</option><option value="12"${runtime.session.tarotCount === 12 ? " selected" : ""}>12 lá · Vòng năm</option><option value="15"${runtime.session.tarotCount === 15 ? " selected" : ""}>15 lá · Bản đồ đầy đủ</option></select></label><label><span>Seed tái tạo (tùy chọn)</span><input type="text" maxlength="120" data-fortune-tarot-seed value="${escapeHtml(runtime.session.tarotSeed || "")}" placeholder="Để trống để tạo seed mới"></label><button class="fortune-primary" type="button" data-fortune-draw>♢ Rút bài</button><small>Cùng seed + cùng số lá sẽ cho cùng kết quả ban đầu; hệ thống lưu provenance của bộ 78 lá.</small></section>${results}`;
  }
  ...
  function astrologyV4Markup(runtime) {
    ...
    const modeOptions = [["natal", "Natal Chart"], ["transit", "Transit"], ["progression", "Secondary Progression + Solar Arc"], ["solar-return", "Solar Return"], ["lunar-return", "Lunar Return"], ["synastry", "Synastry hai hồ sơ"], ["relocation", "Relocation Chart"], ["astrocartography", "Astrocartography"]];
    ...
    else if (result?.ok && mode === "synastry") body = `<p>${aspects.length} góc hợp chéo; không chấm điểm hợp/khắc.</p><div class="fortune-v4-table">${aspects.slice(0, 32).map((aspect) => `<div><span>${escapeHtml(aspect.first)} ${aspect.symbol} ${escapeHtml(aspect.second)}</span><small>${aspect.separation.toFixed(2)}° · orb ${aspect.orb}° · lệch ${aspect.exactness.toFixed(2)}°</small></div>`).join("")}</div>`;
    else if (result?.ok) body = `<div class="fortune-planet-grid">${planets.map((planet) => `<button type="button" data-fortune-planet-detail="${escapeHtml(planet.body)}"><i>${planet.symbol}</i><span>${escapeHtml(planet.name)}</span><b>${escapeHtml(planet.sign.name)} ${planet.sign.degree.toFixed(1)}°</b><small>${planet.house ? `Nhà ${planet.house}` : ""}${planet.retrograde ? " · R" : ""}</small></button>`).join("")}</div><div class="fortune-v4-two-col"><section><header><span>PHÂN BỐ</span></header><pre>${escapeHtml(JSON.stringify(distributionsValue, null, 2))}</pre></section><section><header><span>GÓC HỢP</span></header><div class="fortune-v4-table">${aspects.slice(0, 20).map((aspect) => `<div><span>${escapeHtml(aspect.first)} ${aspect.symbol} ${escapeHtml(aspect.second)}</span><small>${aspect.separation.toFixed(2)}° · orb ${aspect.orb}°</small></div>`).join("") || "<p>Không có góc hợp trong orb đã chọn.</p>"}</div></section></div>`;
    const selectedPlanet = planets.find((planet) => planet.body === runtime.session.astrologyPlanet);
    if (selectedPlanet) body += `<aside class="fortune-planet-inspector"><header><i>${selectedPlanet.symbol}</i><div><small>PLANET INSPECTOR</small><strong>${escapeHtml(selectedPlanet.name)} · ${escapeHtml(selectedPlanet.sign.name)} ${selectedPlanet.sign.degree.toFixed(2)}°</strong></div></header><p>Kinh độ hoàng đạo ${Number(selectedPlanet.longitude).toFixed(5)}°${selectedPlanet.house ? ` · nhà ${selectedPlanet.house}` : ""}${selectedPlanet.retrograde ? " · nghịch hành" : ""}.</p><small>Vị trí là kết quả tính toán thiên văn; cách diễn giải cung và nhà là lớp biểu tượng.</small></aside>`;
    const output = result?.ok ? `<article class="fortune-v4-card"><header><span>${escapeHtml(mode.toUpperCase())}</span><strong>${escapeHtml(result.targetUtc || result.instantUtc || result.returnUtc || result.progressedInstantUtc || "Đã tính")}</strong><div class="fortune-result-actions"><button type="button" data-fortune-astrology-export="json">JSON</button><button type="button" data-fortune-astrology-export="svg">SVG</button></div></header>${body}<div class="fortune-v4-output-tags"><b>TÍNH TOÁN</b><b>BIỂU TƯỢNG</b></div></article>` : `<div class="fortune-empty fortune-empty--compact"><i>◎</i><strong>Chưa có kết quả Astrology Studio</strong><p>${escapeHtml(result?.errors?.[0] || "Chạy engine sau khi hồ sơ có ngày, giờ, timezone IANA, tọa độ và phương pháp rõ ràng.")}</p></div>`;
  ...
  function easternV4Markup(runtime) {
    ...
    const output = result?.ok ? "<article class=\"fortune-v4-card\"><header><span>" + escapeHtml(result.date) + "</span><strong>" + escapeHtml(result.lunarLabel || "Không đọc được lịch âm") + "</strong></header><div class=\"fortune-eastern-hero\"><b>" + escapeHtml(result.yearPillar.stem + " " + result.yearPillar.branch) + "</b><span>Can Chi năm · không phải toàn bộ Bát Tự</span></div><div class=\"fortune-v4-table\">" + result.solarTerms.map((term) => "<div><span>" + escapeHtml(term.name) + " · " + term.longitude + "°</span><small>" + escapeHtml(term.time || "Không có event") + "</small></div>").join("") + "</div><div class=\"fortune-v4-output-tags\"><b>TÍNH TOÁN</b><b>BIỂU TƯỢNG · có giới hạn</b></div></article>" : "<div class=\"fortune-empty fortune-empty--compact\"><i>☯</i><strong>Chưa tính nền lịch phương Đông</strong><p>Chạy công cụ để xem Can Chi năm, lịch âm của môi trường và 24 tiết khí.</p></div>";
  ...
  function symbolsV4Markup(runtime) {
    ...
    const output = result?.cards ? "<article class=\"fortune-v4-card\"><header><span>" + escapeHtml(result.type) + " · seed " + escapeHtml(result.seed) + "</span><strong>" + result.cards.length + " biểu tượng</strong></header><div class=\"fortune-symbol-grid\">" + result.cards.map((card) => "<article><i>" + escapeHtml(card.symbol) + "</i><h3>" + escapeHtml(card.name) + "</h3><p>" + escapeHtml(card.prompt) + "</p></article>").join("") + "</div><div class=\"fortune-v4-output-tags\"><b>BIỂU TƯỢNG</b></div></article>" : "<div class=\"fortune-empty fortune-empty--compact\"><i>✧</i><strong>Chọn một bộ để bắt đầu</strong><p>Toàn bộ mô tả do HH biên soạn, không dùng ảnh bên thứ ba.</p></div>";
    return toolbarMarkup("Thư viện biểu tượng", "Lenormand, Rune và Oracle HH dùng ký hiệu nguyên bản; chỉ là công cụ suy ngẫm.") + "<section class=\"fortune-v4-workspace fortune-symbols-studio\"><article class=\"fortune-v4-card fortune-v4-form\"><div class=\"fortune-v4-inline-fields\"><label><span>Bộ</span><select data-fortune-symbol-type><option value=\"lenormand\"" + (type === "lenormand" ? " selected" : "") + ">Lenormand 36</option><option value=\"runes\"" + (type === "runes" ? " selected" : "") + ">Elder Futhark 24</option><option value=\"oracle\"" + (type === "oracle" ? " selected" : "") + ">Oracle HH 24</option></select></label><label><span>Số lá</span><input type=\"number\" min=\"1\" max=\"9\" data-fortune-symbol-count value=\"" + (runtime.session.symbolCount || 3) + "\"></label><label><span>Seed</span><input type=\"text\" data-fortune-symbol-seed value=\"" + escapeHtml(runtime.session.symbolSeed || "") + "\" placeholder=\"Để trống để tạo mới\"></label></div><button class=\"fortune-primary\" type=\"button\" data-fortune-symbol-draw>Rút biểu tượng</button></article>" + output + "</section>";
  }

  function academyV4Markup(runtime) {
    const v4 = suiteV4(); const quiz = runtime.session.tarotQuiz || v4?.tarotQuiz?.("academy-" + (runtime.session.academyRound || 1)); const stats = v4?.tarotStatistics?.(runtime.session.tarot78 ? [runtime.session.tarot78] : []);
    const quizMarkup = quiz ? "<div class=\"fortune-academy-card\"><i>" + quiz.card.symbol + "</i><p>Điều nào mô tả lớp ánh sáng của lá này theo bộ nội dung HH?</p><div>" + quiz.answers.map((answer, index) => "<button type=\"button\" data-fortune-academy-answer=\"" + index + "\">" + escapeHtml(answer) + "</button>").join("") + "</div><small data-fortune-academy-feedback>" + escapeHtml(runtime.session.academyFeedback || "Hãy tự chọn trước khi mở đáp án.") + "</small></div>" : "";
    return toolbarMarkup("Tarot Academy", "Học 78 lá bằng câu hỏi ngắn, tự diễn giải trước rồi mới mở gợi ý HH.") + "<section class=\"fortune-v4-workspace fortune-academy\"><article class=\"fortune-v4-card\"><header><span>ROUND " + (runtime.session.academyRound || 1) + "</span><strong>" + escapeHtml(quiz?.card?.name || "Chưa có câu hỏi") + "</strong></header>" + quizMarkup + "<button type=\"button\" data-fortune-academy-next>Đổi câu hỏi</button></article><article class=\"fortune-v4-card\"><header><span>TIẾN ĐỘ</span><strong>" + (stats?.total || 0) + " lá trong lịch sử</strong></header><div class=\"fortune-academy-stats\"><div><b>" + (stats?.total || 0) + "</b><span>Lượt đọc</span></div><div><b>" + (stats?.reversedRate || 0) + "%</b><span>Đảo ngược</span></div><div><b>" + (stats?.topCards?.length || 0) + "</b><span>Lá lặp lại</span></div></div><p>Chế độ học không tự lưu câu trả lời; chỉ lưu tiến độ khi người dùng chủ động bật.</p></article></section>";
  }
  ...
  function viewMarkup(runtime) {
    ...
    if (runtime.state.view === "accuracy") return provenanceMarkup(runtime);
    if (runtime.state.view === "tarot") return tarotMarkup(runtime);
    if (runtime.state.view === "academy") return academyV4Markup(runtime);
    ...
    if (runtime.state.view === "sky") return skyV4Markup(runtime);
    if (runtime.state.view === "chart") return astrologyV4Markup(runtime);
    if (runtime.state.view === "eastern") return easternV4Markup(runtime);
    if (runtime.state.view === "symbols") return symbolsV4Markup(runtime);
  ...
  async function handleClick(runtime, event) {
    ...
    if (event.target.closest("[data-fortune-copilot-run]")) {
      ...
      try {
        ...
        runtime.session.copilot = { output: action.output, mode, model: action.model || "Gemini", provider: action.provider || "gemini", usage: action.usage || null, latencyMs: Math.round((globalScope.performance?.now?.() || Date.now()) - startedAt), createdAt: new Date().toISOString() };
    ...
    if (event.target.closest("[data-fortune-astrology-calc]")) {
      ...
      if (!runtime.profile.date || !runtime.profile.time || !runtime.profile.place || !suiteV4()?.timeZoneSupported?.(runtime.profile.timezoneId)) { showToast(runtime, "Cần đủ ngày, giờ, tên địa điểm và timezone IANA hợp lệ trong Hồ sơ phiên.", "error"); return; }
      ...
      if (mode === "synastry" && (!secondDate || !secondTime)) { showToast(runtime, "Synastry cần đủ ngày và giờ của hồ sơ B.", "error"); return; }
      ...
      if (!runtime.session.astrologyV4?.ok) showToast(runtime, runtime.session.astrologyV4?.errors?.[0] || "Không đủ dữ liệu để tính.", "error"); else { playFortuneTone(runtime); showToast(runtime, "Đã tính xong và lưu provenance trong phiên."); }
      render(runtime, true); return;
    }
    const astrologyPlanet = event.target.closest("[data-fortune-planet-detail]");
    if (astrologyPlanet) { runtime.session.astrologyPlanet = astrologyPlanet.dataset.fortunePlanetDetail; render(runtime, true); return; }
    ...
    if (event.target.closest("[data-fortune-symbol-draw]")) {
      const v4 = suiteV4(); runtime.session.symbolType = runtime.root.querySelector("[data-fortune-symbol-type]")?.value || "lenormand"; runtime.session.symbolCount = clamp(runtime.root.querySelector("[data-fortune-symbol-count]")?.value, 1, 9, 3); runtime.session.symbolSeed = runtime.root.querySelector("[data-fortune-symbol-seed]")?.value.trim() || `${Date.now()}-${Math.random()}`;
      runtime.session.symbolDeck = v4?.drawSymbolDeck?.(runtime.session.symbolType, runtime.session.symbolSeed, runtime.session.symbolCount) || null;
      if (!runtime.session.symbolDeck) showToast(runtime, "Symbol deck engine chưa được tải.", "error"); else { playFortuneTone(runtime); addHistory(runtime, "symbols", `${runtime.session.symbolDeck.type} · ${runtime.session.symbolDeck.cards.length} lá`, runtime.session.symbolDeck.cards.map((card) => card.name).join(" · ")); }
      render(runtime, true); return;
    }
    if (event.target.closest("[data-fortune-academy-next]")) { runtime.session.academyRound = Math.max(1, Number(runtime.session.academyRound || 1) + 1); runtime.session.tarotQuiz = suiteV4()?.tarotQuiz?.(`academy-${runtime.session.academyRound}`) || null; runtime.session.academyFeedback = "Hãy tự chọn trước khi mở đáp án."; render(runtime, true); return; }
    ...
    if (academyAnswer) {
      ...
      const selected = Number(academyAnswer.dataset.fortuneAcademyAnswer); runtime.session.academyFeedback = selected === quiz.correctIndex ? "✓ Chính xác. Hãy đọc lại câu trả lời và liên hệ với dữ kiện thật." : `Chưa đúng. Đáp án đúng đã mở: ${quiz.answers[quiz.correctIndex]}`; render(runtime, true); return;
    }
    ...
    if (event.target.closest("[data-fortune-quick-draw]")) {
      ...
      runtime.session.tarotCount = 3; runtime.session.tarotSeed = `${Date.now()}-${Math.random()}`;
      const v4 = suiteV4(); runtime.session.tarot78 = v4?.drawTarot78?.(runtime.session.tarotSeed, { count: 3, allowReversed: true }) || null; runtime.session.tarot = runtime.session.tarot78 ? tarotCardsForView(runtime.session.tarot78) : drawTarot(runtime.session.tarotSeed, 3);
    ...
    if (event.target.closest("[data-fortune-draw]")) {
      ...
      const v4 = suiteV4();
      runtime.session.tarot78 = v4?.drawTarot78?.(runtime.session.tarotSeed, { count: runtime.session.tarotCount, allowReversed: true }) || null;
