(() => {
  ...
  function markup(user) {
    ...
    return `<section class="support-page" data-support-page>
      ...
      <div class="support-main-grid">
        ...
        <aside class="support-bank-card">
          <header><div><span>Bước 2</span><h3>Quét QR chuyển khoản</h3></div><strong>Vietcombank</strong></header>
      ...
      <section class="support-faq"><h3>Câu hỏi thường gặp</h3><details><summary>Khi nào khoản ủng hộ xuất hiện công khai?</summary><p>Sau khi chủ sở hữu đối chiếu giao dịch trong tài khoản Vietcombank và đánh dấu “Đã xác nhận”.</p></details><details><summary>Tại sao số tiền chưa được cộng ngay?</summary><p>Website không có quyền truy cập sao kê ngân hàng. Việc xác nhận thủ công ngăn số liệu giả và bảo vệ tính minh bạch.</p></details><details><summary>Thông tin nào được công khai?</summary><p>Chỉ tên hiển thị, số tiền và lời nhắn. Email, tài khoản đăng nhập và thông tin đối soát không bao giờ xuất hiện trên bảng công khai.</p></details><details><summary>Tôi có thể ủng hộ ẩn danh không?</summary><p>Có. Chọn “Ủng hộ ẩn danh” trước khi tạo mã chuyển khoản.</p></details></section>
  ...
  async function mount(container, options = {}) {
    ...
    page.querySelector("[data-support-form]").addEventListener("submit", async event => {
      event.preventDefault(); const button = event.submitter; button.disabled = true; button.textContent = "Đang tạo mã...";
      try {
        const data = await api("", { method: "POST", body: { action: "create", amount: selectedAmount(), donorName: page.querySelector("[data-support-name]").value, email: page.querySelector("[data-support-email]").value, message: page.querySelector("[data-support-message]").value, anonymous: page.querySelector("[data-support-anonymous]").checked } });
        currentDonation = data.donation; page.querySelector("[data-support-reference]").textContent = currentDonation.reference; page.querySelector("[data-support-copy-reference]").disabled = false; page.querySelector("[data-support-transfer]").hidden = false; page.querySelector("[data-support-transfer-time]").value = new Date(Date.now() - new Date().getTimezoneOffset() * 60000).toISOString().slice(0, 16); setFormStatus(`Đã tạo mã ${currentDonation.reference}. Hãy chuyển đúng nội dung này.`, "success"); page.querySelector("[data-support-bank-amount]").textContent = money(currentDonation.amount); page.querySelector("[data-support-transfer]").scrollIntoView({ behavior: "smooth", block: "center" });
      } catch (error) { setFormStatus(error.message, "error"); }
      finally { button.disabled = false; button.textContent = "Tạo thông tin chuyển khoản"; }
    });
